//----------------------
// <auto-generated>
//     Generated using the NSwag toolchain v13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0)) (http://NSwag.org)
// </auto-generated>
//----------------------

#pragma warning disable 108 // Disable "CS0108 '{derivedDto}.ToJson()' hides inherited member '{dtoBase}.ToJson()'. Use the new keyword if hiding was intended."
#pragma warning disable 114 // Disable "CS0114 '{derivedDto}.RaisePropertyChanged(String)' hides inherited member 'dtoBase.RaisePropertyChanged(String)'. To make the current member override that implementation, add the override keyword. Otherwise add the new keyword."
#pragma warning disable 472 // Disable "CS0472 The result of the expression is always 'false' since a value of type 'Int32' is never equal to 'null' of type 'Int32?'
#pragma warning disable 1573 // Disable "CS1573 Parameter '...' has no matching param tag in the XML comment for ...
#pragma warning disable 1591 // Disable "CS1591 Missing XML comment for publicly visible type or member ..."
#pragma warning disable 8073 // Disable "CS8073 The result of the expression is always 'false' since a value of type 'T' is never equal to 'null' of type 'T?'"
#pragma warning disable 3016 // Disable "CS3016 Arrays as attribute arguments is not CLS-compliant"
#pragma warning disable 8603 // Disable "CS8603 Possible null reference return"

namespace mark.davison.spacetraders.core.Api
{
    using gSystem = global::System;

    [gSystem.CodeDom.Compiler.GeneratedCode("NSwag", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class SpaceTradersApiClient : ISpaceTradersApiClient
    {
        private string _baseUrl = "https://api.spacetraders.io/v2";
        private gSystem.Net.Http.HttpClient _httpClient;
        private gSystem.Lazy<Newtonsoft.Json.JsonSerializerSettings> _settings;

        public SpaceTradersApiClient(gSystem.Net.Http.HttpClient httpClient)
        {
            _httpClient = httpClient;
            _settings = new gSystem.Lazy<Newtonsoft.Json.JsonSerializerSettings>(CreateSerializerSettings);
        }

        private Newtonsoft.Json.JsonSerializerSettings CreateSerializerSettings()
        {
            var settings = new Newtonsoft.Json.JsonSerializerSettings();
            UpdateJsonSerializerSettings(settings);
            return settings;
        }

        public string BaseUrl
        {
            get { return _baseUrl; }
            set { _baseUrl = value; }
        }

        protected Newtonsoft.Json.JsonSerializerSettings JsonSerializerSettings { get { return _settings.Value; } }

        partial void UpdateJsonSerializerSettings(Newtonsoft.Json.JsonSerializerSettings settings);

        partial void PrepareRequest(gSystem.Net.Http.HttpClient client, gSystem.Net.Http.HttpRequestMessage request, string url);
        partial void PrepareRequest(gSystem.Net.Http.HttpClient client, gSystem.Net.Http.HttpRequestMessage request, gSystem.Text.StringBuilder urlBuilder);
        partial void ProcessResponse(gSystem.Net.Http.HttpClient client, gSystem.Net.Http.HttpResponseMessage response);

        /// <summary>
        /// Get Status
        /// </summary>
        /// <remarks>
        /// Return the status of the game server.
        /// <br/>This also includes a few global elements, such as announcements, server reset dates and leaderboards.
        /// </remarks>
        /// <returns>Fetched status successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response> GetStatusAsync()
        {
            return GetStatusAsync(gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Status
        /// </summary>
        /// <remarks>
        /// Return the status of the game server.
        /// <br/>This also includes a few global elements, such as announcements, server reset dates and leaderboards.
        /// </remarks>
        /// <returns>Fetched status successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response> GetStatusAsync(gSystem.Threading.CancellationToken cancellationToken)
        {
            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/");

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Register New Agent
        /// </summary>
        /// <remarks>
        /// Creates a new agent and ties it to an account. 
        /// <br/>The agent symbol must consist of a 3-14 character string, and will be used to represent your agent. This symbol will prefix the symbol of every ship you own. Agent symbols will be cast to all uppercase characters.
        /// <br/>
        /// <br/>This new agent will be tied to a starting faction of your choice, which determines your starting location, and will be granted an authorization token, a contract with their starting faction, a command ship that can fly across space with advanced capabilities, a small probe ship that can be used for reconnaissance, and 150,000 credits.
        /// <br/>
        /// <br/>&gt; #### Keep your token safe and secure
        /// <br/>&gt;
        /// <br/>&gt; Save your token during the alpha phase. There is no way to regenerate this token without starting a new agent. In the future you will be able to generate and manage your tokens from the SpaceTraders website.
        /// <br/>
        /// <br/>If you are new to SpaceTraders, It is recommended to register with the COSMIC faction, a faction that is well connected to the rest of the universe. After registering, you should try our interactive [quickstart guide](https://docs.spacetraders.io/quickstart/new-game) which will walk you through basic API requests in just a few minutes.
        /// </remarks>
        /// <returns>Succesfully registered.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response2> RegisterAsync(RegisterBody body)
        {
            return RegisterAsync(body, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Register New Agent
        /// </summary>
        /// <remarks>
        /// Creates a new agent and ties it to an account. 
        /// <br/>The agent symbol must consist of a 3-14 character string, and will be used to represent your agent. This symbol will prefix the symbol of every ship you own. Agent symbols will be cast to all uppercase characters.
        /// <br/>
        /// <br/>This new agent will be tied to a starting faction of your choice, which determines your starting location, and will be granted an authorization token, a contract with their starting faction, a command ship that can fly across space with advanced capabilities, a small probe ship that can be used for reconnaissance, and 150,000 credits.
        /// <br/>
        /// <br/>&gt; #### Keep your token safe and secure
        /// <br/>&gt;
        /// <br/>&gt; Save your token during the alpha phase. There is no way to regenerate this token without starting a new agent. In the future you will be able to generate and manage your tokens from the SpaceTraders website.
        /// <br/>
        /// <br/>If you are new to SpaceTraders, It is recommended to register with the COSMIC faction, a faction that is well connected to the rest of the universe. After registering, you should try our interactive [quickstart guide](https://docs.spacetraders.io/quickstart/new-game) which will walk you through basic API requests in just a few minutes.
        /// </remarks>
        /// <returns>Succesfully registered.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response2> RegisterAsync(RegisterBody body, gSystem.Threading.CancellationToken cancellationToken)
        {
            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/register");

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response2>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// List Systems
        /// </summary>
        /// <remarks>
        /// Return a paginated list of all systems.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <returns>Successfully listed systems.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response3> GetSystemsAsync(int? page, int? limit)
        {
            return GetSystemsAsync(page, limit, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// List Systems
        /// </summary>
        /// <remarks>
        /// Return a paginated list of all systems.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <returns>Successfully listed systems.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response3> GetSystemsAsync(int? page, int? limit, gSystem.Threading.CancellationToken cancellationToken)
        {
            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/systems?");
            if (page != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("page") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(page, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            if (limit != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("limit") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(limit, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            urlBuilder_.Length--;

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response3>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get System
        /// </summary>
        /// <remarks>
        /// Get the details of a gSystem.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <returns>Successfully fetched the gSystem.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response4> GetSystemAsync(string systemSymbol)
        {
            return GetSystemAsync(systemSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get System
        /// </summary>
        /// <remarks>
        /// Get the details of a gSystem.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <returns>Successfully fetched the gSystem.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response4> GetSystemAsync(string systemSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (systemSymbol == null)
                throw new gSystem.ArgumentNullException("systemSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/systems/{systemSymbol}");
            urlBuilder_.Replace("{systemSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(systemSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response4>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// List Waypoints in System
        /// </summary>
        /// <remarks>
        /// Return a paginated list of all of the waypoints for a given gSystem.
        /// <br/>
        /// <br/>If a waypoint is uncharted, it will return the `Uncharted` trait instead of its actual traits.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <param name="type">Filter waypoints by type.</param>
        /// <param name="traits">Filter waypoints by one or more traits.</param>
        /// <param name="systemSymbol">The system symbol</param>
        /// <returns>Successfully fetched all waypoints in the gSystem.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response5> GetSystemWaypointsAsync(int? page, int? limit, WaypointType? type, WaypointTraitSymbol? traits, string systemSymbol)
        {
            return GetSystemWaypointsAsync(page, limit, type, traits, systemSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// List Waypoints in System
        /// </summary>
        /// <remarks>
        /// Return a paginated list of all of the waypoints for a given gSystem.
        /// <br/>
        /// <br/>If a waypoint is uncharted, it will return the `Uncharted` trait instead of its actual traits.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <param name="type">Filter waypoints by type.</param>
        /// <param name="traits">Filter waypoints by one or more traits.</param>
        /// <param name="systemSymbol">The system symbol</param>
        /// <returns>Successfully fetched all waypoints in the gSystem.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response5> GetSystemWaypointsAsync(int? page, int? limit, WaypointType? type, WaypointTraitSymbol? traits, string systemSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (systemSymbol == null)
                throw new gSystem.ArgumentNullException("systemSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/systems/{systemSymbol}/waypoints?");
            urlBuilder_.Replace("{systemSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(systemSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));
            if (page != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("page") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(page, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            if (limit != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("limit") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(limit, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            if (type != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("type") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(type, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            if (traits != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("traits") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(traits, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            urlBuilder_.Length--;

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response5>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Waypoint
        /// </summary>
        /// <remarks>
        /// View the details of a waypoint.
        /// <br/>
        /// <br/>If the waypoint is uncharted, it will return the 'Uncharted' trait instead of its actual traits.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully fetched waypoint.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response6> GetWaypointAsync(string systemSymbol, string waypointSymbol)
        {
            return GetWaypointAsync(systemSymbol, waypointSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Waypoint
        /// </summary>
        /// <remarks>
        /// View the details of a waypoint.
        /// <br/>
        /// <br/>If the waypoint is uncharted, it will return the 'Uncharted' trait instead of its actual traits.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully fetched waypoint.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response6> GetWaypointAsync(string systemSymbol, string waypointSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (systemSymbol == null)
                throw new gSystem.ArgumentNullException("systemSymbol");

            if (waypointSymbol == null)
                throw new gSystem.ArgumentNullException("waypointSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/systems/{systemSymbol}/waypoints/{waypointSymbol}");
            urlBuilder_.Replace("{systemSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(systemSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));
            urlBuilder_.Replace("{waypointSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(waypointSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response6>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Market
        /// </summary>
        /// <remarks>
        /// Retrieve imports, exports and exchange data from a marketplace. Requires a waypoint that has the `Marketplace` trait to use.
        /// <br/>
        /// <br/>Send a ship to the waypoint to access trade good prices and recent transactions. Refer to the [Market Overview page](https://docs.spacetraders.io/game-concepts/markets) to gain better a understanding of the market in the game.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully fetched the market.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response7> GetMarketAsync(string systemSymbol, string waypointSymbol)
        {
            return GetMarketAsync(systemSymbol, waypointSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Market
        /// </summary>
        /// <remarks>
        /// Retrieve imports, exports and exchange data from a marketplace. Requires a waypoint that has the `Marketplace` trait to use.
        /// <br/>
        /// <br/>Send a ship to the waypoint to access trade good prices and recent transactions. Refer to the [Market Overview page](https://docs.spacetraders.io/game-concepts/markets) to gain better a understanding of the market in the game.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully fetched the market.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response7> GetMarketAsync(string systemSymbol, string waypointSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (systemSymbol == null)
                throw new gSystem.ArgumentNullException("systemSymbol");

            if (waypointSymbol == null)
                throw new gSystem.ArgumentNullException("waypointSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/systems/{systemSymbol}/waypoints/{waypointSymbol}/market");
            urlBuilder_.Replace("{systemSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(systemSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));
            urlBuilder_.Replace("{waypointSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(waypointSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response7>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Shipyard
        /// </summary>
        /// <remarks>
        /// Get the shipyard for a waypoint. Requires a waypoint that has the `Shipyard` trait to use. Send a ship to the waypoint to access data on ships that are currently available for purchase and recent transactions.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully fetched the shipyard.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response8> GetShipyardAsync(string systemSymbol, string waypointSymbol)
        {
            return GetShipyardAsync(systemSymbol, waypointSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Shipyard
        /// </summary>
        /// <remarks>
        /// Get the shipyard for a waypoint. Requires a waypoint that has the `Shipyard` trait to use. Send a ship to the waypoint to access data on ships that are currently available for purchase and recent transactions.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully fetched the shipyard.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response8> GetShipyardAsync(string systemSymbol, string waypointSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (systemSymbol == null)
                throw new gSystem.ArgumentNullException("systemSymbol");

            if (waypointSymbol == null)
                throw new gSystem.ArgumentNullException("waypointSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/systems/{systemSymbol}/waypoints/{waypointSymbol}/shipyard");
            urlBuilder_.Replace("{systemSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(systemSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));
            urlBuilder_.Replace("{waypointSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(waypointSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response8>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Jump Gate
        /// </summary>
        /// <remarks>
        /// Get jump gate details for a waypoint. Requires a waypoint of type `JUMP_GATE` to use.
        /// <br/>
        /// <br/>Waypoints connected to this jump gate can be
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully fetched jump gate.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response9> GetJumpGateAsync(string systemSymbol, string waypointSymbol)
        {
            return GetJumpGateAsync(systemSymbol, waypointSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Jump Gate
        /// </summary>
        /// <remarks>
        /// Get jump gate details for a waypoint. Requires a waypoint of type `JUMP_GATE` to use.
        /// <br/>
        /// <br/>Waypoints connected to this jump gate can be
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully fetched jump gate.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response9> GetJumpGateAsync(string systemSymbol, string waypointSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (systemSymbol == null)
                throw new gSystem.ArgumentNullException("systemSymbol");

            if (waypointSymbol == null)
                throw new gSystem.ArgumentNullException("waypointSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/systems/{systemSymbol}/waypoints/{waypointSymbol}/jump-gate");
            urlBuilder_.Replace("{systemSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(systemSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));
            urlBuilder_.Replace("{waypointSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(waypointSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response9>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Construction Site
        /// </summary>
        /// <remarks>
        /// Get construction details for a waypoint. Requires a waypoint with a property of `isUnderConstruction` to be true.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully fetched construction site.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response10> GetConstructionAsync(string systemSymbol, string waypointSymbol)
        {
            return GetConstructionAsync(systemSymbol, waypointSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Construction Site
        /// </summary>
        /// <remarks>
        /// Get construction details for a waypoint. Requires a waypoint with a property of `isUnderConstruction` to be true.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully fetched construction site.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response10> GetConstructionAsync(string systemSymbol, string waypointSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (systemSymbol == null)
                throw new gSystem.ArgumentNullException("systemSymbol");

            if (waypointSymbol == null)
                throw new gSystem.ArgumentNullException("waypointSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/systems/{systemSymbol}/waypoints/{waypointSymbol}/construction");
            urlBuilder_.Replace("{systemSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(systemSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));
            urlBuilder_.Replace("{waypointSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(waypointSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response10>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Supply Construction Site
        /// </summary>
        /// <remarks>
        /// Supply a construction site with the specified good. Requires a waypoint with a property of `isUnderConstruction` to be true.
        /// <br/>
        /// <br/>The good must be in your ship's cargo. The good will be removed from your ship's cargo and added to the construction site's materials.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully supplied construction site.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response11> SupplyConstructionAsync(Body2 body, string systemSymbol, string waypointSymbol)
        {
            return SupplyConstructionAsync(body, systemSymbol, waypointSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Supply Construction Site
        /// </summary>
        /// <remarks>
        /// Supply a construction site with the specified good. Requires a waypoint with a property of `isUnderConstruction` to be true.
        /// <br/>
        /// <br/>The good must be in your ship's cargo. The good will be removed from your ship's cargo and added to the construction site's materials.
        /// </remarks>
        /// <param name="systemSymbol">The system symbol</param>
        /// <param name="waypointSymbol">The waypoint symbol</param>
        /// <returns>Successfully supplied construction site.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response11> SupplyConstructionAsync(Body2 body, string systemSymbol, string waypointSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (systemSymbol == null)
                throw new gSystem.ArgumentNullException("systemSymbol");

            if (waypointSymbol == null)
                throw new gSystem.ArgumentNullException("waypointSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/systems/{systemSymbol}/waypoints/{waypointSymbol}/construction/supply");
            urlBuilder_.Replace("{systemSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(systemSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));
            urlBuilder_.Replace("{waypointSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(waypointSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response11>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// List Factions
        /// </summary>
        /// <remarks>
        /// Return a paginated list of all the factions in the game.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <returns>Succesfully fetched factions.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response12> GetFactionsAsync(int? page, int? limit)
        {
            return GetFactionsAsync(page, limit, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// List Factions
        /// </summary>
        /// <remarks>
        /// Return a paginated list of all the factions in the game.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <returns>Succesfully fetched factions.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response12> GetFactionsAsync(int? page, int? limit, gSystem.Threading.CancellationToken cancellationToken)
        {
            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/factions?");
            if (page != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("page") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(page, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            if (limit != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("limit") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(limit, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            urlBuilder_.Length--;

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response12>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Faction
        /// </summary>
        /// <remarks>
        /// View the details of a faction.
        /// </remarks>
        /// <param name="factionSymbol">The faction symbol</param>
        /// <returns>Successfully fetched a faction.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response13> GetFactionAsync(string factionSymbol)
        {
            return GetFactionAsync(factionSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Faction
        /// </summary>
        /// <remarks>
        /// View the details of a faction.
        /// </remarks>
        /// <param name="factionSymbol">The faction symbol</param>
        /// <returns>Successfully fetched a faction.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response13> GetFactionAsync(string factionSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (factionSymbol == null)
                throw new gSystem.ArgumentNullException("factionSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/factions/{factionSymbol}");
            urlBuilder_.Replace("{factionSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(factionSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response13>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Agent
        /// </summary>
        /// <remarks>
        /// Fetch your agent's details.
        /// </remarks>
        /// <returns>Successfully fetched agent details.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response14> GetMyAgentAsync()
        {
            return GetMyAgentAsync(gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Agent
        /// </summary>
        /// <remarks>
        /// Fetch your agent's details.
        /// </remarks>
        /// <returns>Successfully fetched agent details.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response14> GetMyAgentAsync(gSystem.Threading.CancellationToken cancellationToken)
        {
            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/agent");

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response14>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// List Agents
        /// </summary>
        /// <remarks>
        /// Fetch agents details.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <returns>Successfully fetched agents details.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response15> GetAgentsAsync(int? page, int? limit)
        {
            return GetAgentsAsync(page, limit, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// List Agents
        /// </summary>
        /// <remarks>
        /// Fetch agents details.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <returns>Successfully fetched agents details.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response15> GetAgentsAsync(int? page, int? limit, gSystem.Threading.CancellationToken cancellationToken)
        {
            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/agents?");
            if (page != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("page") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(page, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            if (limit != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("limit") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(limit, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            urlBuilder_.Length--;

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response15>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Public Agent
        /// </summary>
        /// <remarks>
        /// Fetch agent details.
        /// </remarks>
        /// <param name="agentSymbol">The agent symbol</param>
        /// <returns>Successfully fetched agent details.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response16> GetAgentAsync(string agentSymbol)
        {
            return GetAgentAsync(agentSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Public Agent
        /// </summary>
        /// <remarks>
        /// Fetch agent details.
        /// </remarks>
        /// <param name="agentSymbol">The agent symbol</param>
        /// <returns>Successfully fetched agent details.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response16> GetAgentAsync(string agentSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (agentSymbol == null)
                throw new gSystem.ArgumentNullException("agentSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/agents/{agentSymbol}");
            urlBuilder_.Replace("{agentSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(agentSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response16>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// List Contracts
        /// </summary>
        /// <remarks>
        /// Return a paginated list of all your contracts.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <returns>Succesfully listed contracts.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response17> GetContractsAsync(int? page, int? limit)
        {
            return GetContractsAsync(page, limit, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// List Contracts
        /// </summary>
        /// <remarks>
        /// Return a paginated list of all your contracts.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <returns>Succesfully listed contracts.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response17> GetContractsAsync(int? page, int? limit, gSystem.Threading.CancellationToken cancellationToken)
        {
            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/contracts?");
            if (page != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("page") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(page, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            if (limit != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("limit") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(limit, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            urlBuilder_.Length--;

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response17>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Contract
        /// </summary>
        /// <remarks>
        /// Get the details of a contract by ID.
        /// </remarks>
        /// <param name="contractId">The contract ID</param>
        /// <returns>Successfully fetched contract.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response18> GetContractAsync(string contractId)
        {
            return GetContractAsync(contractId, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Contract
        /// </summary>
        /// <remarks>
        /// Get the details of a contract by ID.
        /// </remarks>
        /// <param name="contractId">The contract ID</param>
        /// <returns>Successfully fetched contract.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response18> GetContractAsync(string contractId, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (contractId == null)
                throw new gSystem.ArgumentNullException("contractId");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/contracts/{contractId}");
            urlBuilder_.Replace("{contractId}", gSystem.Uri.EscapeDataString(ConvertToString(contractId, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response18>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Accept Contract
        /// </summary>
        /// <remarks>
        /// Accept a contract by ID. 
        /// <br/>
        /// <br/>You can only accept contracts that were offered to you, were not accepted yet, and whose deadlines has not passed yet.
        /// </remarks>
        /// <param name="contractId">The contract ID to accept.</param>
        /// <returns>Succesfully accepted contract.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response19> AcceptContractAsync(string contractId)
        {
            return AcceptContractAsync(contractId, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Accept Contract
        /// </summary>
        /// <remarks>
        /// Accept a contract by ID. 
        /// <br/>
        /// <br/>You can only accept contracts that were offered to you, were not accepted yet, and whose deadlines has not passed yet.
        /// </remarks>
        /// <param name="contractId">The contract ID to accept.</param>
        /// <returns>Succesfully accepted contract.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response19> AcceptContractAsync(string contractId, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (contractId == null)
                throw new gSystem.ArgumentNullException("contractId");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/contracts/{contractId}/accept");
            urlBuilder_.Replace("{contractId}", gSystem.Uri.EscapeDataString(ConvertToString(contractId, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Content = new gSystem.Net.Http.StringContent(string.Empty, gSystem.Text.Encoding.UTF8, "application/json");
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response19>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Deliver Cargo to Contract
        /// </summary>
        /// <remarks>
        /// Deliver cargo to a contract.
        /// <br/>
        /// <br/>In order to use this API, a ship must be at the delivery location (denoted in the delivery terms as `destinationSymbol` of a contract) and must have a number of units of a good required by this contract in its cargo.
        /// <br/>
        /// <br/>Cargo that was delivered will be removed from the ship's cargo.
        /// </remarks>
        /// <param name="contractId">The ID of the contract.</param>
        /// <returns>Successfully delivered cargo to contract.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response20> DeliverContractAsync(Body3 body, string contractId)
        {
            return DeliverContractAsync(body, contractId, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Deliver Cargo to Contract
        /// </summary>
        /// <remarks>
        /// Deliver cargo to a contract.
        /// <br/>
        /// <br/>In order to use this API, a ship must be at the delivery location (denoted in the delivery terms as `destinationSymbol` of a contract) and must have a number of units of a good required by this contract in its cargo.
        /// <br/>
        /// <br/>Cargo that was delivered will be removed from the ship's cargo.
        /// </remarks>
        /// <param name="contractId">The ID of the contract.</param>
        /// <returns>Successfully delivered cargo to contract.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response20> DeliverContractAsync(Body3 body, string contractId, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (contractId == null)
                throw new gSystem.ArgumentNullException("contractId");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/contracts/{contractId}/deliver");
            urlBuilder_.Replace("{contractId}", gSystem.Uri.EscapeDataString(ConvertToString(contractId, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response20>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Fulfill Contract
        /// </summary>
        /// <remarks>
        /// Fulfill a contract. Can only be used on contracts that have all of their delivery terms fulfilled.
        /// </remarks>
        /// <param name="contractId">The ID of the contract to fulfill.</param>
        /// <returns>Successfully fulfilled a contract.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response21> FulfillContractAsync(string contractId)
        {
            return FulfillContractAsync(contractId, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Fulfill Contract
        /// </summary>
        /// <remarks>
        /// Fulfill a contract. Can only be used on contracts that have all of their delivery terms fulfilled.
        /// </remarks>
        /// <param name="contractId">The ID of the contract to fulfill.</param>
        /// <returns>Successfully fulfilled a contract.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response21> FulfillContractAsync(string contractId, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (contractId == null)
                throw new gSystem.ArgumentNullException("contractId");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/contracts/{contractId}/fulfill");
            urlBuilder_.Replace("{contractId}", gSystem.Uri.EscapeDataString(ConvertToString(contractId, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Content = new gSystem.Net.Http.StringContent(string.Empty, gSystem.Text.Encoding.UTF8, "application/json");
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response21>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// List Ships
        /// </summary>
        /// <remarks>
        /// Return a paginated list of all of ships under your agent's ownership.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <returns>Succesfully listed ships.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response22> GetMyShipsAsync(int? page, int? limit)
        {
            return GetMyShipsAsync(page, limit, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// List Ships
        /// </summary>
        /// <remarks>
        /// Return a paginated list of all of ships under your agent's ownership.
        /// </remarks>
        /// <param name="page">What entry offset to request</param>
        /// <param name="limit">How many entries to return per page</param>
        /// <returns>Succesfully listed ships.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response22> GetMyShipsAsync(int? page, int? limit, gSystem.Threading.CancellationToken cancellationToken)
        {
            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships?");
            if (page != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("page") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(page, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            if (limit != null)
            {
                urlBuilder_.Append(gSystem.Uri.EscapeDataString("limit") + "=").Append(gSystem.Uri.EscapeDataString(ConvertToString(limit, gSystem.Globalization.CultureInfo.InvariantCulture))).Append("&");
            }
            urlBuilder_.Length--;

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response22>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Purchase Ship
        /// </summary>
        /// <remarks>
        /// Purchase a ship from a Shipyard. In order to use this function, a ship under your agent's ownership must be in a waypoint that has the `Shipyard` trait, and the Shipyard must sell the type of the desired ship.
        /// <br/>
        /// <br/>Shipyards typically offer ship types, which are predefined templates of ships that have dedicated roles. A template comes with a preset of an engine, a reactor, and a frame. It may also include a few modules and mounts.
        /// </remarks>
        /// <returns>Purchased ship successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response23> PurchaseShipAsync(Body4 body)
        {
            return PurchaseShipAsync(body, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Purchase Ship
        /// </summary>
        /// <remarks>
        /// Purchase a ship from a Shipyard. In order to use this function, a ship under your agent's ownership must be in a waypoint that has the `Shipyard` trait, and the Shipyard must sell the type of the desired ship.
        /// <br/>
        /// <br/>Shipyards typically offer ship types, which are predefined templates of ships that have dedicated roles. A template comes with a preset of an engine, a reactor, and a frame. It may also include a few modules and mounts.
        /// </remarks>
        /// <returns>Purchased ship successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response23> PurchaseShipAsync(Body4 body, gSystem.Threading.CancellationToken cancellationToken)
        {
            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships");

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response23>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Ship
        /// </summary>
        /// <remarks>
        /// Retrieve the details of a ship under your agent's ownership.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>Successfully fetched ship.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response24> GetMyShipAsync(string shipSymbol)
        {
            return GetMyShipAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Ship
        /// </summary>
        /// <remarks>
        /// Retrieve the details of a ship under your agent's ownership.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>Successfully fetched ship.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response24> GetMyShipAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response24>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Ship Cargo
        /// </summary>
        /// <remarks>
        /// Retrieve the cargo of a ship under your agent's ownership.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>Successfully fetched ship's cargo.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response25> GetMyShipCargoAsync(string shipSymbol)
        {
            return GetMyShipCargoAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Ship Cargo
        /// </summary>
        /// <remarks>
        /// Retrieve the cargo of a ship under your agent's ownership.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>Successfully fetched ship's cargo.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response25> GetMyShipCargoAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/cargo");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response25>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Orbit Ship
        /// </summary>
        /// <remarks>
        /// Attempt to move your ship into orbit at its current location. The request will only succeed if your ship is capable of moving into orbit at the time of the request.
        /// <br/>
        /// <br/>Orbiting ships are able to do actions that require the ship to be above surface such as navigating or extracting, but cannot access elements in their current waypoint, such as the market or a shipyard.
        /// <br/>
        /// <br/>The endpoint is idempotent - successive calls will succeed even if the ship is already in orbit.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>The ship has successfully moved into orbit at its current location.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response26> OrbitShipAsync(string shipSymbol)
        {
            return OrbitShipAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Orbit Ship
        /// </summary>
        /// <remarks>
        /// Attempt to move your ship into orbit at its current location. The request will only succeed if your ship is capable of moving into orbit at the time of the request.
        /// <br/>
        /// <br/>Orbiting ships are able to do actions that require the ship to be above surface such as navigating or extracting, but cannot access elements in their current waypoint, such as the market or a shipyard.
        /// <br/>
        /// <br/>The endpoint is idempotent - successive calls will succeed even if the ship is already in orbit.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>The ship has successfully moved into orbit at its current location.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response26> OrbitShipAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/orbit");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Content = new gSystem.Net.Http.StringContent(string.Empty, gSystem.Text.Encoding.UTF8, "application/json");
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response26>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Ship Refine
        /// </summary>
        /// <remarks>
        /// Attempt to refine the raw materials on your ship. The request will only succeed if your ship is capable of refining at the time of the request. In order to be able to refine, a ship must have goods that can be refined and have installed a `Refinery` module that can refine it.
        /// <br/>
        /// <br/>When refining, 30 basic goods will be converted into 10 processed goods.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>The ship has successfully refined goods.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response27> ShipRefineAsync(Body5 body, string shipSymbol)
        {
            return ShipRefineAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Ship Refine
        /// </summary>
        /// <remarks>
        /// Attempt to refine the raw materials on your ship. The request will only succeed if your ship is capable of refining at the time of the request. In order to be able to refine, a ship must have goods that can be refined and have installed a `Refinery` module that can refine it.
        /// <br/>
        /// <br/>When refining, 30 basic goods will be converted into 10 processed goods.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>The ship has successfully refined goods.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response27> ShipRefineAsync(Body5 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/refine");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response27>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Create Chart
        /// </summary>
        /// <remarks>
        /// Command a ship to chart the waypoint at its current location.
        /// <br/>
        /// <br/>Most waypoints in the universe are uncharted by default. These waypoints have their traits hidden until they have been charted by a ship.
        /// <br/>
        /// <br/>Charting a waypoint will record your agent as the one who created the chart, and all other agents would also be able to see the waypoint's traits.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>Created</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response28> CreateChartAsync(string shipSymbol)
        {
            return CreateChartAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Create Chart
        /// </summary>
        /// <remarks>
        /// Command a ship to chart the waypoint at its current location.
        /// <br/>
        /// <br/>Most waypoints in the universe are uncharted by default. These waypoints have their traits hidden until they have been charted by a ship.
        /// <br/>
        /// <br/>Charting a waypoint will record your agent as the one who created the chart, and all other agents would also be able to see the waypoint's traits.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>Created</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response28> CreateChartAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/chart");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Content = new gSystem.Net.Http.StringContent(string.Empty, gSystem.Text.Encoding.UTF8, "application/json");
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response28>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Ship Cooldown
        /// </summary>
        /// <remarks>
        /// Retrieve the details of your ship's reactor cooldown. Some actions such as activating your jump drive, scanning, or extracting resources taxes your reactor and results in a cooldown.
        /// <br/>
        /// <br/>Your ship cannot perform additional actions until your cooldown has expired. The duration of your cooldown is relative to the power consumption of the related modules or mounts for the action taken.
        /// <br/>
        /// <br/>Response returns a 204 status code (no-content) when the ship has no cooldown.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>Succesfully fetched ship's cooldown.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response29> GetShipCooldownAsync(string shipSymbol)
        {
            return GetShipCooldownAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Ship Cooldown
        /// </summary>
        /// <remarks>
        /// Retrieve the details of your ship's reactor cooldown. Some actions such as activating your jump drive, scanning, or extracting resources taxes your reactor and results in a cooldown.
        /// <br/>
        /// <br/>Your ship cannot perform additional actions until your cooldown has expired. The duration of your cooldown is relative to the power consumption of the related modules or mounts for the action taken.
        /// <br/>
        /// <br/>Response returns a 204 status code (no-content) when the ship has no cooldown.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>Succesfully fetched ship's cooldown.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response29> GetShipCooldownAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/cooldown");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response29>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        if (status_ == 204)
                        {
                            string responseText_ = (response_.Content == null) ? string.Empty : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("No cooldown.", status_, responseText_, headers_, null);
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Dock Ship
        /// </summary>
        /// <remarks>
        /// Attempt to dock your ship at its current location. Docking will only succeed if your ship is capable of docking at the time of the request.
        /// <br/>
        /// <br/>Docked ships can access elements in their current location, such as the market or a shipyard, but cannot do actions that require the ship to be above surface such as navigating or extracting.
        /// <br/>
        /// <br/>The endpoint is idempotent - successive calls will succeed even if the ship is already docked.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>The ship has successfully docked at its current location.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response30> DockShipAsync(string shipSymbol)
        {
            return DockShipAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Dock Ship
        /// </summary>
        /// <remarks>
        /// Attempt to dock your ship at its current location. Docking will only succeed if your ship is capable of docking at the time of the request.
        /// <br/>
        /// <br/>Docked ships can access elements in their current location, such as the market or a shipyard, but cannot do actions that require the ship to be above surface such as navigating or extracting.
        /// <br/>
        /// <br/>The endpoint is idempotent - successive calls will succeed even if the ship is already docked.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>The ship has successfully docked at its current location.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response30> DockShipAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/dock");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Content = new gSystem.Net.Http.StringContent(string.Empty, gSystem.Text.Encoding.UTF8, "application/json");
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response30>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Create Survey
        /// </summary>
        /// <remarks>
        /// Create surveys on a waypoint that can be extracted such as asteroid fields. A survey focuses on specific types of deposits from the extracted location. When ships extract using this survey, they are guaranteed to procure a high amount of one of the goods in the survey.
        /// <br/>
        /// <br/>In order to use a survey, send the entire survey details in the body of the extract request.
        /// <br/>
        /// <br/>Each survey may have multiple deposits, and if a symbol shows up more than once, that indicates a higher chance of extracting that resource.
        /// <br/>
        /// <br/>Your ship will enter a cooldown after surveying in which it is unable to perform certain actions. Surveys will eventually expire after a period of time or will be exhausted after being extracted several times based on the survey's size. Multiple ships can use the same survey for extraction.
        /// <br/>
        /// <br/>A ship must have the `Surveyor` mount installed in order to use this function.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>Surveys has been created.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response31> CreateSurveyAsync(string shipSymbol)
        {
            return CreateSurveyAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Create Survey
        /// </summary>
        /// <remarks>
        /// Create surveys on a waypoint that can be extracted such as asteroid fields. A survey focuses on specific types of deposits from the extracted location. When ships extract using this survey, they are guaranteed to procure a high amount of one of the goods in the survey.
        /// <br/>
        /// <br/>In order to use a survey, send the entire survey details in the body of the extract request.
        /// <br/>
        /// <br/>Each survey may have multiple deposits, and if a symbol shows up more than once, that indicates a higher chance of extracting that resource.
        /// <br/>
        /// <br/>Your ship will enter a cooldown after surveying in which it is unable to perform certain actions. Surveys will eventually expire after a period of time or will be exhausted after being extracted several times based on the survey's size. Multiple ships can use the same survey for extraction.
        /// <br/>
        /// <br/>A ship must have the `Surveyor` mount installed in order to use this function.
        /// </remarks>
        /// <param name="shipSymbol">The symbol of the ship.</param>
        /// <returns>Surveys has been created.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response31> CreateSurveyAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/survey");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Content = new gSystem.Net.Http.StringContent(string.Empty, gSystem.Text.Encoding.UTF8, "application/json");
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response31>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Extract Resources
        /// </summary>
        /// <remarks>
        /// Extract resources from a waypoint that can be extracted, such as asteroid fields, into your ship. Send an optional survey as the payload to target specific yields.
        /// <br/>
        /// <br/>The ship must be in orbit to be able to extract and must have mining equipments installed that can extract goods, such as the `Gas Siphon` mount for gas-based goods or `Mining Laser` mount for ore-based goods.
        /// <br/>
        /// <br/>The survey property is now deprecated. See the `extract/survey` endpoint for more details.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Extracted successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response32> ExtractResourcesAsync(Body6 body, string shipSymbol)
        {
            return ExtractResourcesAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Extract Resources
        /// </summary>
        /// <remarks>
        /// Extract resources from a waypoint that can be extracted, such as asteroid fields, into your ship. Send an optional survey as the payload to target specific yields.
        /// <br/>
        /// <br/>The ship must be in orbit to be able to extract and must have mining equipments installed that can extract goods, such as the `Gas Siphon` mount for gas-based goods or `Mining Laser` mount for ore-based goods.
        /// <br/>
        /// <br/>The survey property is now deprecated. See the `extract/survey` endpoint for more details.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Extracted successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response32> ExtractResourcesAsync(Body6 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/extract");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response32>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Siphon Resources
        /// </summary>
        /// <remarks>
        /// Siphon gases, such as hydrocarbon, from gas giants.
        /// <br/>
        /// <br/>The ship must be in orbit to be able to siphon and must have siphon mounts and a gas processor installed.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Siphon successful.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response33> SiphonResourcesAsync(string shipSymbol)
        {
            return SiphonResourcesAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Siphon Resources
        /// </summary>
        /// <remarks>
        /// Siphon gases, such as hydrocarbon, from gas giants.
        /// <br/>
        /// <br/>The ship must be in orbit to be able to siphon and must have siphon mounts and a gas processor installed.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Siphon successful.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response33> SiphonResourcesAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/siphon");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Content = new gSystem.Net.Http.StringContent(string.Empty, gSystem.Text.Encoding.UTF8, "application/json");
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response33>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Extract Resources with Survey
        /// </summary>
        /// <remarks>
        /// Use a survey when extracting resources from a waypoint. This endpoint requires a survey as the payload, which allows your ship to extract specific yields.
        /// <br/>
        /// <br/>Send the full survey object as the payload which will be validated according to the signature. If the signature is invalid, or any properties of the survey are changed, the request will fail.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Extracted successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response34> ExtractResourcesWithSurveyAsync(Survey body, string shipSymbol)
        {
            return ExtractResourcesWithSurveyAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Extract Resources with Survey
        /// </summary>
        /// <remarks>
        /// Use a survey when extracting resources from a waypoint. This endpoint requires a survey as the payload, which allows your ship to extract specific yields.
        /// <br/>
        /// <br/>Send the full survey object as the payload which will be validated according to the signature. If the signature is invalid, or any properties of the survey are changed, the request will fail.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Extracted successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response34> ExtractResourcesWithSurveyAsync(Survey body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/extract/survey");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response34>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Jettison Cargo
        /// </summary>
        /// <remarks>
        /// Jettison cargo from your ship's cargo hold.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Jettison successful.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response35> JettisonAsync(Body7 body, string shipSymbol)
        {
            return JettisonAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Jettison Cargo
        /// </summary>
        /// <remarks>
        /// Jettison cargo from your ship's cargo hold.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Jettison successful.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response35> JettisonAsync(Body7 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/jettison");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response35>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Jump Ship
        /// </summary>
        /// <remarks>
        /// Jump your ship instantly to a target connected waypoint. The ship must be in orbit to execute a jump.
        /// <br/>
        /// <br/>A unit of antimatter is purchased and consumed from the market when jumping. The price of antimatter is determined by the market and is subject to change. A ship can only jump to connected waypoints
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Jump successful.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response36> JumpShipAsync(Body8 body, string shipSymbol)
        {
            return JumpShipAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Jump Ship
        /// </summary>
        /// <remarks>
        /// Jump your ship instantly to a target connected waypoint. The ship must be in orbit to execute a jump.
        /// <br/>
        /// <br/>A unit of antimatter is purchased and consumed from the market when jumping. The price of antimatter is determined by the market and is subject to change. A ship can only jump to connected waypoints
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Jump successful.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response36> JumpShipAsync(Body8 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/jump");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response36>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Navigate Ship
        /// </summary>
        /// <remarks>
        /// Navigate to a target destination. The ship must be in orbit to use this function. The destination waypoint must be within the same system as the ship's current location. Navigating will consume the necessary fuel from the ship's manifest based on the distance to the target waypoint.
        /// <br/>
        /// <br/>The returned response will detail the route information including the expected time of arrival. Most ship actions are unavailable until the ship has arrived at it's destination.
        /// <br/>
        /// <br/>To travel between systems, see the ship's Warp or Jump actions.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>The successful transit information including the route details and changes to ship fuel. The route includes the expected time of arrival.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response37> NavigateShipAsync(Body9 body, string shipSymbol)
        {
            return NavigateShipAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Navigate Ship
        /// </summary>
        /// <remarks>
        /// Navigate to a target destination. The ship must be in orbit to use this function. The destination waypoint must be within the same system as the ship's current location. Navigating will consume the necessary fuel from the ship's manifest based on the distance to the target waypoint.
        /// <br/>
        /// <br/>The returned response will detail the route information including the expected time of arrival. Most ship actions are unavailable until the ship has arrived at it's destination.
        /// <br/>
        /// <br/>To travel between systems, see the ship's Warp or Jump actions.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>The successful transit information including the route details and changes to ship fuel. The route includes the expected time of arrival.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response37> NavigateShipAsync(Body9 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/navigate");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response37>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Patch Ship Nav
        /// </summary>
        /// <remarks>
        /// Update the nav configuration of a ship.
        /// <br/>
        /// <br/>Currently only supports configuring the Flight Mode of the ship, which affects its speed and fuel consumption.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>The updated nav data of the ship.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response38> PatchShipNavAsync(Body10 body, string shipSymbol)
        {
            return PatchShipNavAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Patch Ship Nav
        /// </summary>
        /// <remarks>
        /// Update the nav configuration of a ship.
        /// <br/>
        /// <br/>Currently only supports configuring the Flight Mode of the ship, which affects its speed and fuel consumption.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>The updated nav data of the ship.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response38> PatchShipNavAsync(Body10 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/nav");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("PATCH");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response38>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Ship Nav
        /// </summary>
        /// <remarks>
        /// Get the current nav status of a ship.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>The current nav status of the ship.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response39> GetShipNavAsync(string shipSymbol)
        {
            return GetShipNavAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Ship Nav
        /// </summary>
        /// <remarks>
        /// Get the current nav status of a ship.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>The current nav status of the ship.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response39> GetShipNavAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/nav");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response39>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Warp Ship
        /// </summary>
        /// <remarks>
        /// Warp your ship to a target destination in another gSystem. The ship must be in orbit to use this function and must have the `Warp Drive` module installed. Warping will consume the necessary fuel from the ship's manifest.
        /// <br/>
        /// <br/>The returned response will detail the route information including the expected time of arrival. Most ship actions are unavailable until the ship has arrived at its destination.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>The successful transit information including the route details and changes to ship fuel. The route includes the expected time of arrival.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response40> WarpShipAsync(Body11 body, string shipSymbol)
        {
            return WarpShipAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Warp Ship
        /// </summary>
        /// <remarks>
        /// Warp your ship to a target destination in another gSystem. The ship must be in orbit to use this function and must have the `Warp Drive` module installed. Warping will consume the necessary fuel from the ship's manifest.
        /// <br/>
        /// <br/>The returned response will detail the route information including the expected time of arrival. Most ship actions are unavailable until the ship has arrived at its destination.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>The successful transit information including the route details and changes to ship fuel. The route includes the expected time of arrival.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response40> WarpShipAsync(Body11 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/warp");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response40>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Sell Cargo
        /// </summary>
        /// <remarks>
        /// Sell cargo in your ship to a market that trades this cargo. The ship must be docked in a waypoint that has the `Marketplace` trait in order to use this function.
        /// </remarks>
        /// <param name="shipSymbol">Symbol of a ship.</param>
        /// <returns>Cargo was successfully sold.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response41> SellCargoAsync(SellCargoRequest body, string shipSymbol)
        {
            return SellCargoAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Sell Cargo
        /// </summary>
        /// <remarks>
        /// Sell cargo in your ship to a market that trades this cargo. The ship must be docked in a waypoint that has the `Marketplace` trait in order to use this function.
        /// </remarks>
        /// <param name="shipSymbol">Symbol of a ship.</param>
        /// <returns>Cargo was successfully sold.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response41> SellCargoAsync(SellCargoRequest body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/sell");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response41>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Scan Systems
        /// </summary>
        /// <remarks>
        /// Scan for nearby systems, retrieving information on the systems' distance from the ship and their waypoints. Requires a ship to have the `Sensor Array` mount installed to use.
        /// <br/>
        /// <br/>The ship will enter a cooldown after using this function, during which it cannot execute certain actions.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Successfully scanned for nearby systems.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response42> CreateShipSystemScanAsync(string shipSymbol)
        {
            return CreateShipSystemScanAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Scan Systems
        /// </summary>
        /// <remarks>
        /// Scan for nearby systems, retrieving information on the systems' distance from the ship and their waypoints. Requires a ship to have the `Sensor Array` mount installed to use.
        /// <br/>
        /// <br/>The ship will enter a cooldown after using this function, during which it cannot execute certain actions.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Successfully scanned for nearby systems.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response42> CreateShipSystemScanAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/scan/systems");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Content = new gSystem.Net.Http.StringContent(string.Empty, gSystem.Text.Encoding.UTF8, "application/json");
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response42>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Scan Waypoints
        /// </summary>
        /// <remarks>
        /// Scan for nearby waypoints, retrieving detailed information on each waypoint in range. Scanning uncharted waypoints will allow you to ignore their uncharted state and will list the waypoints' traits.
        /// <br/>
        /// <br/>Requires a ship to have the `Sensor Array` mount installed to use.
        /// <br/>
        /// <br/>The ship will enter a cooldown after using this function, during which it cannot execute certain actions.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Successfully scanned for nearby waypoints.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response43> CreateShipWaypointScanAsync(string shipSymbol)
        {
            return CreateShipWaypointScanAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Scan Waypoints
        /// </summary>
        /// <remarks>
        /// Scan for nearby waypoints, retrieving detailed information on each waypoint in range. Scanning uncharted waypoints will allow you to ignore their uncharted state and will list the waypoints' traits.
        /// <br/>
        /// <br/>Requires a ship to have the `Sensor Array` mount installed to use.
        /// <br/>
        /// <br/>The ship will enter a cooldown after using this function, during which it cannot execute certain actions.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Successfully scanned for nearby waypoints.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response43> CreateShipWaypointScanAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/scan/waypoints");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Content = new gSystem.Net.Http.StringContent(string.Empty, gSystem.Text.Encoding.UTF8, "application/json");
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response43>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Scan Ships
        /// </summary>
        /// <remarks>
        /// Scan for nearby ships, retrieving information for all ships in range.
        /// <br/>
        /// <br/>Requires a ship to have the `Sensor Array` mount installed to use.
        /// <br/>
        /// <br/>The ship will enter a cooldown after using this function, during which it cannot execute certain actions.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Successfully scanned for nearby ships.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response44> CreateShipShipScanAsync(string shipSymbol)
        {
            return CreateShipShipScanAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Scan Ships
        /// </summary>
        /// <remarks>
        /// Scan for nearby ships, retrieving information for all ships in range.
        /// <br/>
        /// <br/>Requires a ship to have the `Sensor Array` mount installed to use.
        /// <br/>
        /// <br/>The ship will enter a cooldown after using this function, during which it cannot execute certain actions.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Successfully scanned for nearby ships.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response44> CreateShipShipScanAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/scan/ships");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Content = new gSystem.Net.Http.StringContent(string.Empty, gSystem.Text.Encoding.UTF8, "application/json");
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response44>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Refuel Ship
        /// </summary>
        /// <remarks>
        /// Refuel your ship by buying fuel from the local market.
        /// <br/>
        /// <br/>Requires the ship to be docked in a waypoint that has the `Marketplace` trait, and the market must be selling fuel in order to refuel.
        /// <br/>
        /// <br/>Each fuel bought from the market replenishes 100 units in your ship's fuel.
        /// <br/>
        /// <br/>Ships will always be refuel to their frame's maximum fuel capacity when using this action.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Refueled successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response45> RefuelShipAsync(Body12 body, string shipSymbol)
        {
            return RefuelShipAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Refuel Ship
        /// </summary>
        /// <remarks>
        /// Refuel your ship by buying fuel from the local market.
        /// <br/>
        /// <br/>Requires the ship to be docked in a waypoint that has the `Marketplace` trait, and the market must be selling fuel in order to refuel.
        /// <br/>
        /// <br/>Each fuel bought from the market replenishes 100 units in your ship's fuel.
        /// <br/>
        /// <br/>Ships will always be refuel to their frame's maximum fuel capacity when using this action.
        /// </remarks>
        /// <param name="shipSymbol">The ship symbol.</param>
        /// <returns>Refueled successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response45> RefuelShipAsync(Body12 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/refuel");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response45>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Purchase Cargo
        /// </summary>
        /// <remarks>
        /// Purchase cargo from a market.
        /// <br/>
        /// <br/>The ship must be docked in a waypoint that has `Marketplace` trait, and the market must be selling a good to be able to purchase it.
        /// <br/>
        /// <br/>The maximum amount of units of a good that can be purchased in each transaction are denoted by the `tradeVolume` value of the good, which can be viewed by using the Get Market action.
        /// <br/>
        /// <br/>Purchased goods are added to the ship's cargo hold.
        /// </remarks>
        /// <param name="shipSymbol">The ship's symbol.</param>
        /// <returns>Purchased goods successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response46> PurchaseCargoAsync(Body13 body, string shipSymbol)
        {
            return PurchaseCargoAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Purchase Cargo
        /// </summary>
        /// <remarks>
        /// Purchase cargo from a market.
        /// <br/>
        /// <br/>The ship must be docked in a waypoint that has `Marketplace` trait, and the market must be selling a good to be able to purchase it.
        /// <br/>
        /// <br/>The maximum amount of units of a good that can be purchased in each transaction are denoted by the `tradeVolume` value of the good, which can be viewed by using the Get Market action.
        /// <br/>
        /// <br/>Purchased goods are added to the ship's cargo hold.
        /// </remarks>
        /// <param name="shipSymbol">The ship's symbol.</param>
        /// <returns>Purchased goods successfully.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response46> PurchaseCargoAsync(Body13 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/purchase");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response46>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Transfer Cargo
        /// </summary>
        /// <remarks>
        /// Transfer cargo between ships.
        /// <br/>
        /// <br/>The receiving ship must be in the same waypoint as the transferring ship, and it must able to hold the additional cargo after the transfer is complete. Both ships also must be in the same state, either both are docked or both are orbiting.
        /// <br/>
        /// <br/>The response body's cargo shows the cargo of the transferring ship after the transfer is complete.
        /// </remarks>
        /// <param name="shipSymbol">The transferring ship's symbol.</param>
        /// <returns>Transfer successful.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response47> TransferCargoAsync(Body14 body, string shipSymbol)
        {
            return TransferCargoAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Transfer Cargo
        /// </summary>
        /// <remarks>
        /// Transfer cargo between ships.
        /// <br/>
        /// <br/>The receiving ship must be in the same waypoint as the transferring ship, and it must able to hold the additional cargo after the transfer is complete. Both ships also must be in the same state, either both are docked or both are orbiting.
        /// <br/>
        /// <br/>The response body's cargo shows the cargo of the transferring ship after the transfer is complete.
        /// </remarks>
        /// <param name="shipSymbol">The transferring ship's symbol.</param>
        /// <returns>Transfer successful.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response47> TransferCargoAsync(Body14 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/transfer");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response47>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Negotiate Contract
        /// </summary>
        /// <remarks>
        /// Negotiate a new contract with the HQ.
        /// <br/>
        /// <br/>In order to negotiate a new contract, an agent must not have ongoing or offered contracts over the allowed maximum amount. Currently the maximum contracts an agent can have at a time is 1.
        /// <br/>
        /// <br/>Once a contract is negotiated, it is added to the list of contracts offered to the agent, which the agent can then accept. 
        /// <br/>
        /// <br/>The ship must be present at any waypoint with a faction present to negotiate a contract with that faction.
        /// </remarks>
        /// <param name="shipSymbol">The ship's symbol.</param>
        /// <returns>Successfully negotiated a new contract.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response48> NegotiateContractAsync(string shipSymbol)
        {
            return NegotiateContractAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Negotiate Contract
        /// </summary>
        /// <remarks>
        /// Negotiate a new contract with the HQ.
        /// <br/>
        /// <br/>In order to negotiate a new contract, an agent must not have ongoing or offered contracts over the allowed maximum amount. Currently the maximum contracts an agent can have at a time is 1.
        /// <br/>
        /// <br/>Once a contract is negotiated, it is added to the list of contracts offered to the agent, which the agent can then accept. 
        /// <br/>
        /// <br/>The ship must be present at any waypoint with a faction present to negotiate a contract with that faction.
        /// </remarks>
        /// <param name="shipSymbol">The ship's symbol.</param>
        /// <returns>Successfully negotiated a new contract.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response48> NegotiateContractAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/negotiate/contract");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Content = new gSystem.Net.Http.StringContent(string.Empty, gSystem.Text.Encoding.UTF8, "application/json");
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response48>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Get Mounts
        /// </summary>
        /// <remarks>
        /// Get the mounts installed on a ship.
        /// </remarks>
        /// <param name="shipSymbol">The ship's symbol.</param>
        /// <returns>Got installed mounts.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response49> GetMountsAsync(string shipSymbol)
        {
            return GetMountsAsync(shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Get Mounts
        /// </summary>
        /// <remarks>
        /// Get the mounts installed on a ship.
        /// </remarks>
        /// <param name="shipSymbol">The ship's symbol.</param>
        /// <returns>Got installed mounts.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response49> GetMountsAsync(string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/mounts");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    request_.Method = new gSystem.Net.Http.HttpMethod("GET");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 200)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response49>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Install Mount
        /// </summary>
        /// <remarks>
        /// Install a mount on a ship.
        /// <br/>
        /// <br/>In order to install a mount, the ship must be docked and located in a waypoint that has a `Shipyard` trait. The ship also must have the mount to install in its cargo hold.
        /// <br/>
        /// <br/>An installation fee will be deduced by the Shipyard for installing the mount on the ship.
        /// </remarks>
        /// <param name="shipSymbol">The ship's symbol.</param>
        /// <returns>Successfully installed the mount.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response50> InstallMountAsync(Body15 body, string shipSymbol)
        {
            return InstallMountAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Install Mount
        /// </summary>
        /// <remarks>
        /// Install a mount on a ship.
        /// <br/>
        /// <br/>In order to install a mount, the ship must be docked and located in a waypoint that has a `Shipyard` trait. The ship also must have the mount to install in its cargo hold.
        /// <br/>
        /// <br/>An installation fee will be deduced by the Shipyard for installing the mount on the ship.
        /// </remarks>
        /// <param name="shipSymbol">The ship's symbol.</param>
        /// <returns>Successfully installed the mount.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response50> InstallMountAsync(Body15 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/mounts/install");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response50>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        /// <summary>
        /// Remove Mount
        /// </summary>
        /// <remarks>
        /// Remove a mount from a ship.
        /// <br/>
        /// <br/>The ship must be docked in a waypoint that has the `Shipyard` trait, and must have the desired mount that it wish to remove installed.
        /// <br/>
        /// <br/>A removal fee will be deduced from the agent by the Shipyard.
        /// </remarks>
        /// <param name="shipSymbol">The ship's symbol.</param>
        /// <returns>Successfully removed the mount.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual gSystem.Threading.Tasks.Task<Response51> RemoveMountAsync(Body16 body, string shipSymbol)
        {
            return RemoveMountAsync(body, shipSymbol, gSystem.Threading.CancellationToken.None);
        }

        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <summary>
        /// Remove Mount
        /// </summary>
        /// <remarks>
        /// Remove a mount from a ship.
        /// <br/>
        /// <br/>The ship must be docked in a waypoint that has the `Shipyard` trait, and must have the desired mount that it wish to remove installed.
        /// <br/>
        /// <br/>A removal fee will be deduced from the agent by the Shipyard.
        /// </remarks>
        /// <param name="shipSymbol">The ship's symbol.</param>
        /// <returns>Successfully removed the mount.</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async gSystem.Threading.Tasks.Task<Response51> RemoveMountAsync(Body16 body, string shipSymbol, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (shipSymbol == null)
                throw new gSystem.ArgumentNullException("shipSymbol");

            var urlBuilder_ = new gSystem.Text.StringBuilder();
            urlBuilder_.Append(BaseUrl != null ? BaseUrl.TrimEnd('/') : "").Append("/my/ships/{shipSymbol}/mounts/remove");
            urlBuilder_.Replace("{shipSymbol}", gSystem.Uri.EscapeDataString(ConvertToString(shipSymbol, gSystem.Globalization.CultureInfo.InvariantCulture)));

            var client_ = _httpClient;
            var disposeClient_ = false;
            try
            {
                using (var request_ = new gSystem.Net.Http.HttpRequestMessage())
                {
                    var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(body, _settings.Value);
                    var content_ = new gSystem.Net.Http.StringContent(json_);
                    content_.Headers.ContentType = gSystem.Net.Http.Headers.MediaTypeHeaderValue.Parse("application/json");
                    request_.Content = content_;
                    request_.Method = new gSystem.Net.Http.HttpMethod("POST");
                    request_.Headers.Accept.Add(gSystem.Net.Http.Headers.MediaTypeWithQualityHeaderValue.Parse("application/json"));

                    PrepareRequest(client_, request_, urlBuilder_);

                    var url_ = urlBuilder_.ToString();
                    request_.RequestUri = new gSystem.Uri(url_, gSystem.UriKind.RelativeOrAbsolute);

                    PrepareRequest(client_, request_, url_);

                    var response_ = await client_.SendAsync(request_, gSystem.Net.Http.HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false);
                    var disposeResponse_ = true;
                    try
                    {
                        var headers_ = gSystem.Linq.Enumerable.ToDictionary(response_.Headers, h_ => h_.Key, h_ => h_.Value);
                        if (response_.Content != null && response_.Content.Headers != null)
                        {
                            foreach (var item_ in response_.Content.Headers)
                                headers_[item_.Key] = item_.Value;
                        }

                        ProcessResponse(client_, response_);

                        var status_ = (int)response_.StatusCode;
                        if (status_ == 201)
                        {
                            var objectResponse_ = await ReadObjectResponseAsync<Response51>(response_, headers_, cancellationToken).ConfigureAwait(false);
                            if (objectResponse_.Object == null)
                            {
                                throw new ApiException("Response was null which was not expected.", status_, objectResponse_.Text, headers_, null);
                            }
                            return objectResponse_.Object;
                        }
                        else
                        {
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").", status_, responseData_, headers_, null);
                        }
                    }
                    finally
                    {
                        if (disposeResponse_)
                            response_.Dispose();
                    }
                }
            }
            finally
            {
                if (disposeClient_)
                    client_.Dispose();
            }
        }

        protected struct ObjectResponseResult<T>
        {
            public ObjectResponseResult(T responseObject, string responseText)
            {
                this.Object = responseObject;
                this.Text = responseText;
            }

            public T Object { get; }

            public string Text { get; }
        }

        public bool ReadResponseAsString { get; set; }

        protected virtual async gSystem.Threading.Tasks.Task<ObjectResponseResult<T>> ReadObjectResponseAsync<T>(gSystem.Net.Http.HttpResponseMessage response, gSystem.Collections.Generic.IReadOnlyDictionary<string, gSystem.Collections.Generic.IEnumerable<string>> headers, gSystem.Threading.CancellationToken cancellationToken)
        {
            if (response == null || response.Content == null)
            {
                return new ObjectResponseResult<T>(default(T), string.Empty);
            }

            if (ReadResponseAsString)
            {
                var responseText = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
                try
                {
                    var typedBody = Newtonsoft.Json.JsonConvert.DeserializeObject<T>(responseText, JsonSerializerSettings);
                    return new ObjectResponseResult<T>(typedBody, responseText);
                }
                catch (Newtonsoft.Json.JsonException exception)
                {
                    var message = "Could not deserialize the response body string as " + typeof(T).FullName + ".";
                    throw new ApiException(message, (int)response.StatusCode, responseText, headers, exception);
                }
            }
            else
            {
                try
                {
                    using (var responseStream = await response.Content.ReadAsStreamAsync().ConfigureAwait(false))
                    using (var streamReader = new gSystem.IO.StreamReader(responseStream))
                    using (var jsonTextReader = new Newtonsoft.Json.JsonTextReader(streamReader))
                    {
                        var serializer = Newtonsoft.Json.JsonSerializer.Create(JsonSerializerSettings);
                        var typedBody = serializer.Deserialize<T>(jsonTextReader);
                        return new ObjectResponseResult<T>(typedBody, string.Empty);
                    }
                }
                catch (Newtonsoft.Json.JsonException exception)
                {
                    var message = "Could not deserialize the response body stream as " + typeof(T).FullName + ".";
                    throw new ApiException(message, (int)response.StatusCode, string.Empty, headers, exception);
                }
            }
        }

        private string ConvertToString(object value, gSystem.Globalization.CultureInfo cultureInfo)
        {
            if (value == null)
            {
                return "";
            }

            if (value is gSystem.Enum)
            {
                var name = gSystem.Enum.GetName(value.GetType(), value);
                if (name != null)
                {
                    var field = gSystem.Reflection.IntrospectionExtensions.GetTypeInfo(value.GetType()).GetDeclaredField(name);
                    if (field != null)
                    {
                        var attribute = gSystem.Reflection.CustomAttributeExtensions.GetCustomAttribute(field, typeof(gSystem.Runtime.Serialization.EnumMemberAttribute))
                            as gSystem.Runtime.Serialization.EnumMemberAttribute;
                        if (attribute != null)
                        {
                            return attribute.Value != null ? attribute.Value : name;
                        }
                    }

                    var converted = gSystem.Convert.ToString(gSystem.Convert.ChangeType(value, gSystem.Enum.GetUnderlyingType(value.GetType()), cultureInfo));
                    return converted == null ? string.Empty : converted;
                }
            }
            else if (value is bool)
            {
                return gSystem.Convert.ToString((bool)value, cultureInfo).ToLowerInvariant();
            }
            else if (value is byte[])
            {
                return gSystem.Convert.ToBase64String((byte[])value);
            }
            else if (value.GetType().IsArray)
            {
                var array = gSystem.Linq.Enumerable.OfType<object>((gSystem.Array)value);
                return string.Join(",", gSystem.Linq.Enumerable.Select(array, o => ConvertToString(o, cultureInfo)));
            }

            var result = gSystem.Convert.ToString(value, cultureInfo);
            return result == null ? "" : result;
        }
    }

    /// <summary>
    /// The activity level of a trade good. If the good is an import, this represents how strong consumption is. If the good is an export, this represents how strong the production is for the good. When activity is strong, consumption or production is near maximum capacity. When activity is weak, consumption or production is near minimum capacity.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ActivityLevel
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"WEAK")]
        WEAK = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"GROWING")]
        GROWING = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"STRONG")]
        STRONG = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"RESTRICTED")]
        RESTRICTED = 3,

    }

    /// <summary>
    /// Agent details.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Agent
    {
        /// <summary>
        /// Account ID that is tied to this agent. Only included on your own agent.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("accountId", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string AccountId { get; set; }

        /// <summary>
        /// Symbol of the agent.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        [gSystem.ComponentModel.DataAnnotations.StringLength(14, MinimumLength = 3)]
        public string Symbol { get; set; }

        /// <summary>
        /// The headquarters of the agent.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("headquarters", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Headquarters { get; set; }

        /// <summary>
        /// The number of credits the agent has available. Credits can be negative if funds have been overdrawn.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("credits", Required = Newtonsoft.Json.Required.Always)]
        public long Credits { get; set; }

        /// <summary>
        /// The faction the agent started with.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("startingFaction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string StartingFaction { get; set; }

        /// <summary>
        /// How many ships are owned by the agent.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipCount", Required = Newtonsoft.Json.Required.Always)]
        public int ShipCount { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The chart of a system or waypoint, which makes the location visible to other agents.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Chart
    {
        [Newtonsoft.Json.JsonProperty("waypointSymbol", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string WaypointSymbol { get; set; }

        /// <summary>
        /// The agent that submitted the chart for this waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("submittedBy", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public string SubmittedBy { get; set; }

        /// <summary>
        /// The time the chart for this waypoint was submitted.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("submittedOn", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public gSystem.DateTimeOffset SubmittedOn { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The construction details of a waypoint.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Construction
    {
        /// <summary>
        /// The symbol of the waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Symbol { get; set; }

        /// <summary>
        /// The materials required to construct the waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("materials", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ConstructionMaterial> Materials { get; set; } = new gSystem.Collections.ObjectModel.Collection<ConstructionMaterial>();

        /// <summary>
        /// Whether the waypoint has been constructed.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("isComplete", Required = Newtonsoft.Json.Required.Always)]
        public bool IsComplete { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The details of the required construction materials for a given waypoint under construction.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ConstructionMaterial
    {
        [Newtonsoft.Json.JsonProperty("tradeSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public TradeSymbol TradeSymbol { get; set; }

        /// <summary>
        /// The number of units required.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("required", Required = Newtonsoft.Json.Required.Always)]
        public int Required { get; set; }

        /// <summary>
        /// The number of units fulfilled toward the required amount.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("fulfilled", Required = Newtonsoft.Json.Required.Always)]
        public int Fulfilled { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Contract details.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Contract
    {
        /// <summary>
        /// ID of the contract.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("id", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Id { get; set; }

        /// <summary>
        /// The symbol of the faction that this contract is for.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("factionSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string FactionSymbol { get; set; }

        /// <summary>
        /// Type of contract.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("type", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ContractType Type { get; set; }

        [Newtonsoft.Json.JsonProperty("terms", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ContractTerms Terms { get; set; } = new ContractTerms();

        /// <summary>
        /// Whether the contract has been accepted by the agent
        /// </summary>
        [Newtonsoft.Json.JsonProperty("accepted", Required = Newtonsoft.Json.Required.Always)]
        public bool Accepted { get; set; } = false;

        /// <summary>
        /// Whether the contract has been fulfilled
        /// </summary>
        [Newtonsoft.Json.JsonProperty("fulfilled", Required = Newtonsoft.Json.Required.Always)]
        public bool Fulfilled { get; set; } = false;

        /// <summary>
        /// Deprecated in favor of deadlineToAccept
        /// </summary>
        [Newtonsoft.Json.JsonProperty("expiration", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.Obsolete]
        public gSystem.DateTimeOffset Expiration { get; set; }

        /// <summary>
        /// The time at which the contract is no longer available to be accepted
        /// </summary>
        [Newtonsoft.Json.JsonProperty("deadlineToAccept", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public gSystem.DateTimeOffset DeadlineToAccept { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The details of a delivery contract. Includes the type of good, units needed, and the destination.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ContractDeliverGood
    {
        /// <summary>
        /// The symbol of the trade good to deliver.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("tradeSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string TradeSymbol { get; set; }

        /// <summary>
        /// The destination where goods need to be delivered.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("destinationSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string DestinationSymbol { get; set; }

        /// <summary>
        /// The number of units that need to be delivered on this contract.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("unitsRequired", Required = Newtonsoft.Json.Required.Always)]
        public int UnitsRequired { get; set; }

        /// <summary>
        /// The number of units fulfilled on this contract.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("unitsFulfilled", Required = Newtonsoft.Json.Required.Always)]
        public int UnitsFulfilled { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Payments for the contract.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ContractPayment
    {
        /// <summary>
        /// The amount of credits received up front for accepting the contract.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("onAccepted", Required = Newtonsoft.Json.Required.Always)]
        public int OnAccepted { get; set; }

        /// <summary>
        /// The amount of credits received when the contract is fulfilled.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("onFulfilled", Required = Newtonsoft.Json.Required.Always)]
        public int OnFulfilled { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The terms to fulfill the contract.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ContractTerms
    {
        /// <summary>
        /// The deadline for the contract.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("deadline", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public gSystem.DateTimeOffset Deadline { get; set; }

        [Newtonsoft.Json.JsonProperty("payment", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ContractPayment Payment { get; set; } = new ContractPayment();

        /// <summary>
        /// The cargo that needs to be delivered to fulfill the contract.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("deliver", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public gSystem.Collections.Generic.ICollection<ContractDeliverGood> Deliver { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// A cooldown is a period of time in which a ship cannot perform certain actions.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Cooldown
    {
        /// <summary>
        /// The symbol of the ship that is on cooldown
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string ShipSymbol { get; set; }

        /// <summary>
        /// The total duration of the cooldown in seconds
        /// </summary>
        [Newtonsoft.Json.JsonProperty("totalSeconds", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int TotalSeconds { get; set; }

        /// <summary>
        /// The remaining duration of the cooldown in seconds
        /// </summary>
        [Newtonsoft.Json.JsonProperty("remainingSeconds", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int RemainingSeconds { get; set; }

        /// <summary>
        /// The date and time when the cooldown expires in ISO 8601 format
        /// </summary>
        [Newtonsoft.Json.JsonProperty("expiration", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public gSystem.DateTimeOffset Expiration { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Extraction details.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Extraction
    {
        /// <summary>
        /// Symbol of the ship that executed the extraction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string ShipSymbol { get; set; }

        [Newtonsoft.Json.JsonProperty("yield", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ExtractionYield Yield { get; set; } = new ExtractionYield();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// A yield from the extraction operation.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ExtractionYield
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public TradeSymbol Symbol { get; set; }

        /// <summary>
        /// The number of units extracted that were placed into the ship's cargo hold.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        public int Units { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Faction details.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Faction
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public FactionSymbol Symbol { get; set; }

        /// <summary>
        /// Name of the faction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Name { get; set; }

        /// <summary>
        /// Description of the faction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Description { get; set; }

        /// <summary>
        /// The waypoint in which the faction's HQ is located in.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("headquarters", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Headquarters { get; set; }

        /// <summary>
        /// List of traits that define this faction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("traits", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<FactionTrait> Traits { get; set; } = new gSystem.Collections.ObjectModel.Collection<FactionTrait>();

        /// <summary>
        /// Whether or not the faction is currently recruiting new agents.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("isRecruiting", Required = Newtonsoft.Json.Required.Always)]
        public bool IsRecruiting { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The symbol of the faction.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum FactionSymbol
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"COSMIC")]
        COSMIC = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"VOID")]
        VOID = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"GALACTIC")]
        GALACTIC = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"QUANTUM")]
        QUANTUM = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DOMINION")]
        DOMINION = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ASTRO")]
        ASTRO = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CORSAIRS")]
        CORSAIRS = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"OBSIDIAN")]
        OBSIDIAN = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"AEGIS")]
        AEGIS = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"UNITED")]
        UNITED = 9,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SOLITARY")]
        SOLITARY = 10,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"COBALT")]
        COBALT = 11,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"OMEGA")]
        OMEGA = 12,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ECHO")]
        ECHO = 13,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"LORDS")]
        LORDS = 14,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CULT")]
        CULT = 15,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ANCIENTS")]
        ANCIENTS = 16,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHADOW")]
        SHADOW = 17,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ETHEREAL")]
        ETHEREAL = 18,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class FactionTrait
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public FactionTraitSymbol Symbol { get; set; }

        /// <summary>
        /// The name of the trait.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        /// <summary>
        /// A description of the trait.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Description { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The unique identifier of the trait.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum FactionTraitSymbol
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BUREAUCRATIC")]
        BUREAUCRATIC = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SECRETIVE")]
        SECRETIVE = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CAPITALISTIC")]
        CAPITALISTIC = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"INDUSTRIOUS")]
        INDUSTRIOUS = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PEACEFUL")]
        PEACEFUL = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DISTRUSTFUL")]
        DISTRUSTFUL = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"WELCOMING")]
        WELCOMING = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SMUGGLERS")]
        SMUGGLERS = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SCAVENGERS")]
        SCAVENGERS = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REBELLIOUS")]
        REBELLIOUS = 9,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXILES")]
        EXILES = 10,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PIRATES")]
        PIRATES = 11,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"RAIDERS")]
        RAIDERS = 12,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CLAN")]
        CLAN = 13,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"GUILD")]
        GUILD = 14,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DOMINION")]
        DOMINION = 15,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRINGE")]
        FRINGE = 16,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FORSAKEN")]
        FORSAKEN = 17,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ISOLATED")]
        ISOLATED = 18,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"LOCALIZED")]
        LOCALIZED = 19,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ESTABLISHED")]
        ESTABLISHED = 20,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"NOTABLE")]
        NOTABLE = 21,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DOMINANT")]
        DOMINANT = 22,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"INESCAPABLE")]
        INESCAPABLE = 23,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"INNOVATIVE")]
        INNOVATIVE = 24,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BOLD")]
        BOLD = 25,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"VISIONARY")]
        VISIONARY = 26,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CURIOUS")]
        CURIOUS = 27,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DARING")]
        DARING = 28,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXPLORATORY")]
        EXPLORATORY = 29,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"RESOURCEFUL")]
        RESOURCEFUL = 30,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FLEXIBLE")]
        FLEXIBLE = 31,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"COOPERATIVE")]
        COOPERATIVE = 32,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"UNITED")]
        UNITED = 33,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"STRATEGIC")]
        STRATEGIC = 34,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"INTELLIGENT")]
        INTELLIGENT = 35,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"RESEARCH_FOCUSED")]
        RESEARCH_FOCUSED = 36,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"COLLABORATIVE")]
        COLLABORATIVE = 37,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PROGRESSIVE")]
        PROGRESSIVE = 38,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MILITARISTIC")]
        MILITARISTIC = 39,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"TECHNOLOGICALLY_ADVANCED")]
        TECHNOLOGICALLY_ADVANCED = 40,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"AGGRESSIVE")]
        AGGRESSIVE = 41,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"IMPERIALISTIC")]
        IMPERIALISTIC = 42,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"TREASURE_HUNTERS")]
        TREASURE_HUNTERS = 43,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DEXTEROUS")]
        DEXTEROUS = 44,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"UNPREDICTABLE")]
        UNPREDICTABLE = 45,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BRUTAL")]
        BRUTAL = 46,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FLEETING")]
        FLEETING = 47,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ADAPTABLE")]
        ADAPTABLE = 48,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SELF_SUFFICIENT")]
        SELF_SUFFICIENT = 49,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DEFENSIVE")]
        DEFENSIVE = 50,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PROUD")]
        PROUD = 51,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DIVERSE")]
        DIVERSE = 52,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"INDEPENDENT")]
        INDEPENDENT = 53,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SELF_INTERESTED")]
        SELF_INTERESTED = 54,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAGMENTED")]
        FRAGMENTED = 55,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"COMMERCIAL")]
        COMMERCIAL = 56,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FREE_MARKETS")]
        FREE_MARKETS = 57,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ENTREPRENEURIAL")]
        ENTREPRENEURIAL = 58,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class JumpGate
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string Symbol { get; set; }

        /// <summary>
        /// All the gates that are connected to this waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("connections", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<string> Connections { get; set; } = new gSystem.Collections.ObjectModel.Collection<string>();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Market
    {
        /// <summary>
        /// The symbol of the market. The symbol is the same as the waypoint where the market is located.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Symbol { get; set; }

        /// <summary>
        /// The list of goods that are exported from this market.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("exports", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<TradeGood> Exports { get; set; } = new gSystem.Collections.ObjectModel.Collection<TradeGood>();

        /// <summary>
        /// The list of goods that are sought as imports in this market.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("imports", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<TradeGood> Imports { get; set; } = new gSystem.Collections.ObjectModel.Collection<TradeGood>();

        /// <summary>
        /// The list of goods that are bought and sold between agents at this market.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("exchange", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<TradeGood> Exchange { get; set; } = new gSystem.Collections.ObjectModel.Collection<TradeGood>();

        /// <summary>
        /// The list of recent transactions at this market. Visible only when a ship is present at the market.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("transactions", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public gSystem.Collections.Generic.ICollection<MarketTransaction> Transactions { get; set; }

        /// <summary>
        /// The list of goods that are traded at this market. Visible only when a ship is present at the market.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("tradeGoods", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public gSystem.Collections.Generic.ICollection<MarketTradeGood> TradeGoods { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class MarketTradeGood
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public TradeSymbol Symbol { get; set; }

        /// <summary>
        /// The type of trade good (export, import, or exchange).
        /// </summary>
        [Newtonsoft.Json.JsonProperty("type", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public MarketTradeGoodType Type { get; set; }

        /// <summary>
        /// This is the maximum number of units that can be purchased or sold at this market in a single trade for this good. Trade volume also gives an indication of price volatility. A market with a low trade volume will have large price swings, while high trade volume will be more resilient to price changes.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("tradeVolume", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(1, int.MaxValue)]
        public int TradeVolume { get; set; }

        [Newtonsoft.Json.JsonProperty("supply", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public SupplyLevel Supply { get; set; }

        [Newtonsoft.Json.JsonProperty("activity", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ActivityLevel Activity { get; set; }

        /// <summary>
        /// The price at which this good can be purchased from the market.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("purchasePrice", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int PurchasePrice { get; set; }

        /// <summary>
        /// The price at which this good can be sold to the market.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("sellPrice", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int SellPrice { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Result of a transaction with a market.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class MarketTransaction
    {
        [Newtonsoft.Json.JsonProperty("waypointSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string WaypointSymbol { get; set; }

        /// <summary>
        /// The symbol of the ship that made the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string ShipSymbol { get; set; }

        /// <summary>
        /// The symbol of the trade good.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("tradeSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string TradeSymbol { get; set; }

        /// <summary>
        /// The type of transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("type", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public MarketTransactionType Type { get; set; }

        /// <summary>
        /// The number of units of the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Units { get; set; }

        /// <summary>
        /// The price per unit of the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("pricePerUnit", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int PricePerUnit { get; set; }

        /// <summary>
        /// The total price of the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("totalPrice", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int TotalPrice { get; set; }

        /// <summary>
        /// The timestamp of the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("timestamp", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public gSystem.DateTimeOffset Timestamp { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Meta details for pagination.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Meta
    {
        /// <summary>
        /// Shows the total amount of items of this kind that exist.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("total", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Total { get; set; }

        /// <summary>
        /// A page denotes an amount of items, offset from the first item. Each page holds an amount of items equal to the `limit`.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("page", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(1, int.MaxValue)]
        public int Page { get; set; } = 1;

        /// <summary>
        /// The amount of items in each page. Limits how many items can be fetched at once.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("limit", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(1, 20)]
        public int Limit { get; set; } = 10;

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The ship that was scanned. Details include information about the ship that could be detected by the scanner.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ScannedShip
    {
        /// <summary>
        /// The globally unique identifier of the ship.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Symbol { get; set; }

        [Newtonsoft.Json.JsonProperty("registration", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipRegistration Registration { get; set; } = new ShipRegistration();

        [Newtonsoft.Json.JsonProperty("nav", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNav Nav { get; set; } = new ShipNav();

        /// <summary>
        /// The frame of the ship.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("frame", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public Frame Frame { get; set; }

        /// <summary>
        /// The reactor of the ship.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("reactor", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public Reactor Reactor { get; set; }

        /// <summary>
        /// The engine of the ship.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("engine", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Engine Engine { get; set; } = new Engine();

        /// <summary>
        /// List of mounts installed in the ship.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("mounts", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public gSystem.Collections.Generic.ICollection<Mounts> Mounts { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Details of a system was that scanned.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ScannedSystem
    {
        /// <summary>
        /// Symbol of the gSystem.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Symbol { get; set; }

        /// <summary>
        /// Symbol of the system's sector.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("sectorSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string SectorSymbol { get; set; }

        [Newtonsoft.Json.JsonProperty("type", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public SystemType Type { get; set; }

        /// <summary>
        /// Position in the universe in the x axis.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("x", Required = Newtonsoft.Json.Required.Always)]
        public int X { get; set; }

        /// <summary>
        /// Position in the universe in the y axis.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("y", Required = Newtonsoft.Json.Required.Always)]
        public int Y { get; set; }

        /// <summary>
        /// The system's distance from the scanning ship.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("distance", Required = Newtonsoft.Json.Required.Always)]
        public int Distance { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// A waypoint that was scanned by a ship.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ScannedWaypoint
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string Symbol { get; set; }

        [Newtonsoft.Json.JsonProperty("type", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public WaypointType Type { get; set; }

        [Newtonsoft.Json.JsonProperty("systemSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string SystemSymbol { get; set; }

        /// <summary>
        /// Position in the universe in the x axis.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("x", Required = Newtonsoft.Json.Required.Always)]
        public int X { get; set; }

        /// <summary>
        /// Position in the universe in the y axis.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("y", Required = Newtonsoft.Json.Required.Always)]
        public int Y { get; set; }

        /// <summary>
        /// List of waypoints that orbit this waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("orbitals", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<WaypointOrbital> Orbitals { get; set; } = new gSystem.Collections.ObjectModel.Collection<WaypointOrbital>();

        [Newtonsoft.Json.JsonProperty("faction", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public WaypointFaction Faction { get; set; }

        /// <summary>
        /// The traits of the waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("traits", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<WaypointTrait> Traits { get; set; } = new gSystem.Collections.ObjectModel.Collection<WaypointTrait>();

        [Newtonsoft.Json.JsonProperty("chart", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public Chart Chart { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Ship details.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Ship
    {
        /// <summary>
        /// The globally unique identifier of the ship in the following format: `[AGENT_SYMBOL]-[HEX_ID]`
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Symbol { get; set; }

        [Newtonsoft.Json.JsonProperty("registration", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipRegistration Registration { get; set; } = new ShipRegistration();

        [Newtonsoft.Json.JsonProperty("nav", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNav Nav { get; set; } = new ShipNav();

        [Newtonsoft.Json.JsonProperty("crew", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCrew Crew { get; set; } = new ShipCrew();

        [Newtonsoft.Json.JsonProperty("frame", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipFrame Frame { get; set; } = new ShipFrame();

        [Newtonsoft.Json.JsonProperty("reactor", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipReactor Reactor { get; set; } = new ShipReactor();

        [Newtonsoft.Json.JsonProperty("engine", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipEngine Engine { get; set; } = new ShipEngine();

        [Newtonsoft.Json.JsonProperty("cooldown", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Cooldown Cooldown { get; set; } = new Cooldown();

        /// <summary>
        /// Modules installed in this ship.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("modules", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ShipModule> Modules { get; set; } = new gSystem.Collections.ObjectModel.Collection<ShipModule>();

        /// <summary>
        /// Mounts installed in this ship.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("mounts", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ShipMount> Mounts { get; set; } = new gSystem.Collections.ObjectModel.Collection<ShipMount>();

        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        [Newtonsoft.Json.JsonProperty("fuel", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipFuel Fuel { get; set; } = new ShipFuel();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Ship cargo details.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipCargo
    {
        /// <summary>
        /// The max number of items that can be stored in the cargo hold.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("capacity", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Capacity { get; set; }

        /// <summary>
        /// The number of items currently stored in the cargo hold.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Units { get; set; }

        /// <summary>
        /// The items currently in the cargo hold.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("inventory", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ShipCargoItem> Inventory { get; set; } = new gSystem.Collections.ObjectModel.Collection<ShipCargoItem>();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The type of cargo item and the number of units.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipCargoItem
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public TradeSymbol Symbol { get; set; }

        /// <summary>
        /// The name of the cargo item type.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        /// <summary>
        /// The description of the cargo item type.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Description { get; set; }

        /// <summary>
        /// The number of units of the cargo item.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(1, int.MaxValue)]
        public int Units { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The ship's crew service and maintain the ship's systems and equipment.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipCrew
    {
        /// <summary>
        /// The current number of crew members on the ship.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("current", Required = Newtonsoft.Json.Required.Always)]
        public int Current { get; set; }

        /// <summary>
        /// The minimum number of crew members required to maintain the ship.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("required", Required = Newtonsoft.Json.Required.Always)]
        public int Required { get; set; }

        /// <summary>
        /// The maximum number of crew members the ship can support.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("capacity", Required = Newtonsoft.Json.Required.Always)]
        public int Capacity { get; set; }

        /// <summary>
        /// The rotation of crew shifts. A stricter shift improves the ship's performance. A more relaxed shift improves the crew's morale.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("rotation", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipCrewRotation Rotation { get; set; } = mark.davison.spacetraders.core.Api.ShipCrewRotation.STRICT;

        /// <summary>
        /// A rough measure of the crew's morale. A higher morale means the crew is happier and more productive. A lower morale means the ship is more prone to accidents.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("morale", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, 100)]
        public int Morale { get; set; }

        /// <summary>
        /// The amount of credits per crew member paid per hour. Wages are paid when a ship docks at a civilized waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("wages", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Wages { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The engine determines how quickly a ship travels between waypoints.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipEngine
    {
        /// <summary>
        /// The symbol of the engine.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipEngineSymbol Symbol { get; set; }

        /// <summary>
        /// The name of the engine.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        /// <summary>
        /// The description of the engine.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Description { get; set; }

        [Newtonsoft.Json.JsonProperty("condition", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, 100)]
        public int Condition { get; set; }

        /// <summary>
        /// The speed stat of this engine. The higher the speed, the faster a ship can travel from one point to another. Reduces the time of arrival when navigating the ship.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("speed", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(1, int.MaxValue)]
        public int Speed { get; set; }

        [Newtonsoft.Json.JsonProperty("requirements", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipRequirements Requirements { get; set; } = new ShipRequirements();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The frame of the ship. The frame determines the number of modules and mounting points of the ship, as well as base fuel capacity. As the condition of the frame takes more wear, the ship will become more sluggish and less maneuverable.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipFrame
    {
        /// <summary>
        /// Symbol of the frame.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipFrameSymbol Symbol { get; set; }

        /// <summary>
        /// Name of the frame.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        /// <summary>
        /// Description of the frame.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Description { get; set; }

        [Newtonsoft.Json.JsonProperty("condition", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, 100)]
        public int Condition { get; set; }

        /// <summary>
        /// The amount of slots that can be dedicated to modules installed in the ship. Each installed module take up a number of slots, and once there are no more slots, no new modules can be installed.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("moduleSlots", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int ModuleSlots { get; set; }

        /// <summary>
        /// The amount of slots that can be dedicated to mounts installed in the ship. Each installed mount takes up a number of points, and once there are no more points remaining, no new mounts can be installed.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("mountingPoints", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int MountingPoints { get; set; }

        /// <summary>
        /// The maximum amount of fuel that can be stored in this ship. When refueling, the ship will be refueled to this amount.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("fuelCapacity", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int FuelCapacity { get; set; }

        [Newtonsoft.Json.JsonProperty("requirements", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipRequirements Requirements { get; set; } = new ShipRequirements();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Details of the ship's fuel tanks including how much fuel was consumed during the last transit or action.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipFuel
    {
        /// <summary>
        /// The current amount of fuel in the ship's tanks.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("current", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Current { get; set; }

        /// <summary>
        /// The maximum amount of fuel the ship's tanks can hold.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("capacity", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Capacity { get; set; }

        /// <summary>
        /// An object that only shows up when an action has consumed fuel in the process. Shows the fuel consumption data.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("consumed", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public Consumed Consumed { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Result of a transaction for a ship modification, such as installing a mount or a module.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipModificationTransaction
    {
        /// <summary>
        /// The symbol of the waypoint where the transaction took place.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("waypointSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string WaypointSymbol { get; set; }

        /// <summary>
        /// The symbol of the ship that made the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string ShipSymbol { get; set; }

        /// <summary>
        /// The symbol of the trade good.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("tradeSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string TradeSymbol { get; set; }

        /// <summary>
        /// The total price of the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("totalPrice", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int TotalPrice { get; set; }

        /// <summary>
        /// The timestamp of the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("timestamp", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public gSystem.DateTimeOffset Timestamp { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// A module can be installed in a ship and provides a set of capabilities such as storage space or quarters for crew. Module installations are permanent.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipModule
    {
        /// <summary>
        /// The symbol of the module.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipModuleSymbol Symbol { get; set; }

        /// <summary>
        /// Modules that provide capacity, such as cargo hold or crew quarters will show this value to denote how much of a bonus the module grants.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("capacity", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Capacity { get; set; }

        /// <summary>
        /// Modules that have a range will such as a sensor array show this value to denote how far can the module reach with its capabilities.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("range", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Range { get; set; }

        /// <summary>
        /// Name of this module.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        /// <summary>
        /// Description of this module.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Description { get; set; }

        [Newtonsoft.Json.JsonProperty("requirements", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipRequirements Requirements { get; set; } = new ShipRequirements();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// A mount is installed on the exterier of a ship.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipMount
    {
        /// <summary>
        /// Symbo of this mount.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipMountSymbol Symbol { get; set; }

        /// <summary>
        /// Name of this mount.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        /// <summary>
        /// Description of this mount.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public string Description { get; set; }

        /// <summary>
        /// Mounts that have this value, such as mining lasers, denote how powerful this mount's capabilities are.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("strength", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Strength { get; set; }

        /// <summary>
        /// Mounts that have this value denote what goods can be produced from using the mount.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("deposits", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore, ItemConverterType = typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public gSystem.Collections.Generic.ICollection<Deposits> Deposits { get; set; }

        [Newtonsoft.Json.JsonProperty("requirements", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipRequirements Requirements { get; set; } = new ShipRequirements();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The navigation information of the ship.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipNav
    {
        [Newtonsoft.Json.JsonProperty("systemSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string SystemSymbol { get; set; }

        [Newtonsoft.Json.JsonProperty("waypointSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string WaypointSymbol { get; set; }

        [Newtonsoft.Json.JsonProperty("route", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNavRoute Route { get; set; } = new ShipNavRoute();

        [Newtonsoft.Json.JsonProperty("status", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipNavStatus Status { get; set; }

        [Newtonsoft.Json.JsonProperty("flightMode", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipNavFlightMode FlightMode { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The ship's set speed when traveling between waypoints or systems.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ShipNavFlightMode
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DRIFT")]
        DRIFT = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"STEALTH")]
        STEALTH = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CRUISE")]
        CRUISE = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BURN")]
        BURN = 3,

    }

    /// <summary>
    /// The routing information for the ship's most recent transit or current location.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipNavRoute
    {
        [Newtonsoft.Json.JsonProperty("destination", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNavRouteWaypoint Destination { get; set; } = new ShipNavRouteWaypoint();

        [Newtonsoft.Json.JsonProperty("origin", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNavRouteWaypoint Origin { get; set; } = new ShipNavRouteWaypoint();

        /// <summary>
        /// The date time of the ship's departure.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("departureTime", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public gSystem.DateTimeOffset DepartureTime { get; set; }

        /// <summary>
        /// The date time of the ship's arrival. If the ship is in-transit, this is the expected time of arrival.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("arrival", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public gSystem.DateTimeOffset Arrival { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The destination or departure of a ships nav route.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipNavRouteWaypoint
    {
        /// <summary>
        /// The symbol of the waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Symbol { get; set; }

        [Newtonsoft.Json.JsonProperty("type", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public WaypointType Type { get; set; }

        [Newtonsoft.Json.JsonProperty("systemSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string SystemSymbol { get; set; }

        /// <summary>
        /// Position in the universe in the x axis.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("x", Required = Newtonsoft.Json.Required.Always)]
        public int X { get; set; }

        /// <summary>
        /// Position in the universe in the y axis.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("y", Required = Newtonsoft.Json.Required.Always)]
        public int Y { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The current status of the ship
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ShipNavStatus
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"IN_TRANSIT")]
        IN_TRANSIT = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"IN_ORBIT")]
        IN_ORBIT = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DOCKED")]
        DOCKED = 2,

    }

    /// <summary>
    /// The reactor of the ship. The reactor is responsible for powering the ship's systems and weapons.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipReactor
    {
        /// <summary>
        /// Symbol of the reactor.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipReactorSymbol Symbol { get; set; }

        /// <summary>
        /// Name of the reactor.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        /// <summary>
        /// Description of the reactor.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Description { get; set; }

        [Newtonsoft.Json.JsonProperty("condition", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, 100)]
        public int Condition { get; set; }

        /// <summary>
        /// The amount of power provided by this reactor. The more power a reactor provides to the ship, the lower the cooldown it gets when using a module or mount that taxes the ship's power.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("powerOutput", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(1, int.MaxValue)]
        public int PowerOutput { get; set; }

        [Newtonsoft.Json.JsonProperty("requirements", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipRequirements Requirements { get; set; } = new ShipRequirements();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The public registration information of the ship
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipRegistration
    {
        /// <summary>
        /// The agent's registered name of the ship
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Name { get; set; }

        /// <summary>
        /// The symbol of the faction the ship is registered with
        /// </summary>
        [Newtonsoft.Json.JsonProperty("factionSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string FactionSymbol { get; set; }

        [Newtonsoft.Json.JsonProperty("role", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipRole Role { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The requirements for installation on a ship
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipRequirements
    {
        /// <summary>
        /// The amount of power required from the reactor.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("power", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public int Power { get; set; }

        /// <summary>
        /// The number of crew required for operation.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("crew", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public int Crew { get; set; }

        /// <summary>
        /// The number of module slots required for installation.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("slots", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public int Slots { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The registered role of the ship
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ShipRole
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FABRICATOR")]
        FABRICATOR = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"HARVESTER")]
        HARVESTER = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"HAULER")]
        HAULER = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"INTERCEPTOR")]
        INTERCEPTOR = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXCAVATOR")]
        EXCAVATOR = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"TRANSPORT")]
        TRANSPORT = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REPAIR")]
        REPAIR = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SURVEYOR")]
        SURVEYOR = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"COMMAND")]
        COMMAND = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CARRIER")]
        CARRIER = 9,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PATROL")]
        PATROL = 10,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SATELLITE")]
        SATELLITE = 11,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXPLORER")]
        EXPLORER = 12,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REFINERY")]
        REFINERY = 13,

    }

    /// <summary>
    /// Type of ship
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ShipType
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_PROBE")]
        SHIP_PROBE = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_MINING_DRONE")]
        SHIP_MINING_DRONE = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_SIPHON_DRONE")]
        SHIP_SIPHON_DRONE = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_INTERCEPTOR")]
        SHIP_INTERCEPTOR = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_LIGHT_HAULER")]
        SHIP_LIGHT_HAULER = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_COMMAND_FRIGATE")]
        SHIP_COMMAND_FRIGATE = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_EXPLORER")]
        SHIP_EXPLORER = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_HEAVY_FREIGHTER")]
        SHIP_HEAVY_FREIGHTER = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_LIGHT_SHUTTLE")]
        SHIP_LIGHT_SHUTTLE = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_ORE_HOUND")]
        SHIP_ORE_HOUND = 9,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_REFINING_FREIGHTER")]
        SHIP_REFINING_FREIGHTER = 10,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_SURVEYOR")]
        SHIP_SURVEYOR = 11,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Shipyard
    {
        /// <summary>
        /// The symbol of the shipyard. The symbol is the same as the waypoint where the shipyard is located.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Symbol { get; set; }

        /// <summary>
        /// The list of ship types available for purchase at this shipyard.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipTypes", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ShipTypes> ShipTypes { get; set; } = new gSystem.Collections.ObjectModel.Collection<ShipTypes>();

        /// <summary>
        /// The list of recent transactions at this shipyard.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("transactions", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public gSystem.Collections.Generic.ICollection<ShipyardTransaction> Transactions { get; set; }

        /// <summary>
        /// The ships that are currently available for purchase at the shipyard.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("ships", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public gSystem.Collections.Generic.ICollection<ShipyardShip> Ships { get; set; }

        /// <summary>
        /// The fee to modify a ship at this shipyard. This includes installing or removing modules and mounts on a ship. In the case of mounts, the fee is a flat rate per mount. In the case of modules, the fee is per slot the module occupies.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("modificationsFee", Required = Newtonsoft.Json.Required.Always)]
        public int ModificationsFee { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipyardShip
    {
        [Newtonsoft.Json.JsonProperty("type", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipType Type { get; set; }

        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Description { get; set; }

        [Newtonsoft.Json.JsonProperty("supply", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public SupplyLevel Supply { get; set; }

        [Newtonsoft.Json.JsonProperty("activity", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ActivityLevel Activity { get; set; }

        [Newtonsoft.Json.JsonProperty("purchasePrice", Required = Newtonsoft.Json.Required.Always)]
        public int PurchasePrice { get; set; }

        [Newtonsoft.Json.JsonProperty("frame", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipFrame Frame { get; set; } = new ShipFrame();

        [Newtonsoft.Json.JsonProperty("reactor", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipReactor Reactor { get; set; } = new ShipReactor();

        [Newtonsoft.Json.JsonProperty("engine", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipEngine Engine { get; set; } = new ShipEngine();

        [Newtonsoft.Json.JsonProperty("modules", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ShipModule> Modules { get; set; } = new gSystem.Collections.ObjectModel.Collection<ShipModule>();

        [Newtonsoft.Json.JsonProperty("mounts", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ShipMount> Mounts { get; set; } = new gSystem.Collections.ObjectModel.Collection<ShipMount>();

        [Newtonsoft.Json.JsonProperty("crew", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Crew Crew { get; set; } = new Crew();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Results of a transaction with a shipyard.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipyardTransaction
    {
        [Newtonsoft.Json.JsonProperty("waypointSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string WaypointSymbol { get; set; }

        /// <summary>
        /// The symbol of the ship that was the subject of the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.Obsolete]
        public string ShipSymbol { get; set; }

        /// <summary>
        /// The symbol of the ship that was the subject of the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipType", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string ShipType { get; set; }

        /// <summary>
        /// The price of the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("price", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Price { get; set; }

        /// <summary>
        /// The symbol of the agent that made the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("agentSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string AgentSymbol { get; set; }

        /// <summary>
        /// The timestamp of the transaction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("timestamp", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public gSystem.DateTimeOffset Timestamp { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// Siphon details.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Siphon
    {
        /// <summary>
        /// Symbol of the ship that executed the siphon.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string ShipSymbol { get; set; }

        [Newtonsoft.Json.JsonProperty("yield", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public SiphonYield Yield { get; set; } = new SiphonYield();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// A yield from the siphon operation.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class SiphonYield
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public TradeSymbol Symbol { get; set; }

        /// <summary>
        /// The number of units siphoned that were placed into the ship's cargo hold.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        public int Units { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The supply level of a trade good.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum SupplyLevel
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SCARCE")]
        SCARCE = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"LIMITED")]
        LIMITED = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODERATE")]
        MODERATE = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"HIGH")]
        HIGH = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ABUNDANT")]
        ABUNDANT = 4,

    }

    /// <summary>
    /// A resource survey of a waypoint, detailing a specific extraction location and the types of resources that can be found there.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Survey
    {
        /// <summary>
        /// A unique signature for the location of this survey. This signature is verified when attempting an extraction using this survey.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("signature", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Signature { get; set; }

        /// <summary>
        /// The symbol of the waypoint that this survey is for.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Symbol { get; set; }

        /// <summary>
        /// A list of deposits that can be found at this location. A ship will extract one of these deposits when using this survey in an extraction request. If multiple deposits of the same type are present, the chance of extracting that deposit is increased.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("deposits", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<SurveyDeposit> Deposits { get; set; } = new gSystem.Collections.ObjectModel.Collection<SurveyDeposit>();

        /// <summary>
        /// The date and time when the survey expires. After this date and time, the survey will no longer be available for extraction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("expiration", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public gSystem.DateTimeOffset Expiration { get; set; }

        /// <summary>
        /// The size of the deposit. This value indicates how much can be extracted from the survey before it is exhausted.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("size", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public SurveySize Size { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// A surveyed deposit of a mineral or resource available for extraction.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class SurveyDeposit
    {
        /// <summary>
        /// The symbol of the deposit.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Symbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class System
    {
        /// <summary>
        /// The symbol of the gSystem.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Symbol { get; set; }

        /// <summary>
        /// The symbol of the sector.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("sectorSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string SectorSymbol { get; set; }

        [Newtonsoft.Json.JsonProperty("type", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public SystemType Type { get; set; }

        /// <summary>
        /// Relative position of the system in the sector in the x axis.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("x", Required = Newtonsoft.Json.Required.Always)]
        public int X { get; set; }

        /// <summary>
        /// Relative position of the system in the sector in the y axis.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("y", Required = Newtonsoft.Json.Required.Always)]
        public int Y { get; set; }

        /// <summary>
        /// Waypoints in this gSystem.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("waypoints", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<SystemWaypoint> Waypoints { get; set; } = new gSystem.Collections.ObjectModel.Collection<SystemWaypoint>();

        /// <summary>
        /// Factions that control this gSystem.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("factions", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<SystemFaction> Factions { get; set; } = new gSystem.Collections.ObjectModel.Collection<SystemFaction>();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class SystemFaction
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public FactionSymbol Symbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The type of gSystem.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum SystemType
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"NEUTRON_STAR")]
        NEUTRON_STAR = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"RED_STAR")]
        RED_STAR = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ORANGE_STAR")]
        ORANGE_STAR = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BLUE_STAR")]
        BLUE_STAR = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"YOUNG_STAR")]
        YOUNG_STAR = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"WHITE_DWARF")]
        WHITE_DWARF = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BLACK_HOLE")]
        BLACK_HOLE = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"HYPERGIANT")]
        HYPERGIANT = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"NEBULA")]
        NEBULA = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"UNSTABLE")]
        UNSTABLE = 9,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class SystemWaypoint
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string Symbol { get; set; }

        [Newtonsoft.Json.JsonProperty("type", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public WaypointType Type { get; set; }

        /// <summary>
        /// Relative position of the waypoint on the system's x axis. This is not an absolute position in the universe.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("x", Required = Newtonsoft.Json.Required.Always)]
        public int X { get; set; }

        /// <summary>
        /// Relative position of the waypoint on the system's y axis. This is not an absolute position in the universe.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("y", Required = Newtonsoft.Json.Required.Always)]
        public int Y { get; set; }

        /// <summary>
        /// Waypoints that orbit this waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("orbitals", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<WaypointOrbital> Orbitals { get; set; } = new gSystem.Collections.ObjectModel.Collection<WaypointOrbital>();

        /// <summary>
        /// The symbol of the parent waypoint, if this waypoint is in orbit around another waypoint. Otherwise this value is undefined.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("orbits", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string Orbits { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// A good that can be traded for other goods or currency.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class TradeGood
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public TradeSymbol Symbol { get; set; }

        /// <summary>
        /// The name of the good.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        /// <summary>
        /// The description of the good.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Description { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The good's symbol.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum TradeSymbol
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PRECIOUS_STONES")]
        PRECIOUS_STONES = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"QUARTZ_SAND")]
        QUARTZ_SAND = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SILICON_CRYSTALS")]
        SILICON_CRYSTALS = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"AMMONIA_ICE")]
        AMMONIA_ICE = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"LIQUID_HYDROGEN")]
        LIQUID_HYDROGEN = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"LIQUID_NITROGEN")]
        LIQUID_NITROGEN = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ICE_WATER")]
        ICE_WATER = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXOTIC_MATTER")]
        EXOTIC_MATTER = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ADVANCED_CIRCUITRY")]
        ADVANCED_CIRCUITRY = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"GRAVITON_EMITTERS")]
        GRAVITON_EMITTERS = 9,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"IRON")]
        IRON = 10,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"IRON_ORE")]
        IRON_ORE = 11,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"COPPER")]
        COPPER = 12,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"COPPER_ORE")]
        COPPER_ORE = 13,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ALUMINUM")]
        ALUMINUM = 14,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ALUMINUM_ORE")]
        ALUMINUM_ORE = 15,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SILVER")]
        SILVER = 16,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SILVER_ORE")]
        SILVER_ORE = 17,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"GOLD")]
        GOLD = 18,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"GOLD_ORE")]
        GOLD_ORE = 19,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PLATINUM")]
        PLATINUM = 20,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PLATINUM_ORE")]
        PLATINUM_ORE = 21,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DIAMONDS")]
        DIAMONDS = 22,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"URANITE")]
        URANITE = 23,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"URANITE_ORE")]
        URANITE_ORE = 24,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MERITIUM")]
        MERITIUM = 25,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MERITIUM_ORE")]
        MERITIUM_ORE = 26,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"HYDROCARBON")]
        HYDROCARBON = 27,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ANTIMATTER")]
        ANTIMATTER = 28,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FAB_MATS")]
        FAB_MATS = 29,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FERTILIZERS")]
        FERTILIZERS = 30,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FABRICS")]
        FABRICS = 31,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FOOD")]
        FOOD = 32,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"JEWELRY")]
        JEWELRY = 33,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MACHINERY")]
        MACHINERY = 34,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FIREARMS")]
        FIREARMS = 35,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ASSAULT_RIFLES")]
        ASSAULT_RIFLES = 36,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MILITARY_EQUIPMENT")]
        MILITARY_EQUIPMENT = 37,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXPLOSIVES")]
        EXPLOSIVES = 38,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"LAB_INSTRUMENTS")]
        LAB_INSTRUMENTS = 39,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"AMMUNITION")]
        AMMUNITION = 40,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ELECTRONICS")]
        ELECTRONICS = 41,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_PLATING")]
        SHIP_PLATING = 42,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_PARTS")]
        SHIP_PARTS = 43,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EQUIPMENT")]
        EQUIPMENT = 44,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FUEL")]
        FUEL = 45,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MEDICINE")]
        MEDICINE = 46,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DRUGS")]
        DRUGS = 47,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CLOTHING")]
        CLOTHING = 48,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MICROPROCESSORS")]
        MICROPROCESSORS = 49,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PLASTICS")]
        PLASTICS = 50,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"POLYNUCLEOTIDES")]
        POLYNUCLEOTIDES = 51,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BIOCOMPOSITES")]
        BIOCOMPOSITES = 52,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"QUANTUM_STABILIZERS")]
        QUANTUM_STABILIZERS = 53,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"NANOBOTS")]
        NANOBOTS = 54,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"AI_MAINFRAMES")]
        AI_MAINFRAMES = 55,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"QUANTUM_DRIVES")]
        QUANTUM_DRIVES = 56,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ROBOTIC_DRONES")]
        ROBOTIC_DRONES = 57,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CYBER_IMPLANTS")]
        CYBER_IMPLANTS = 58,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"GENE_THERAPEUTICS")]
        GENE_THERAPEUTICS = 59,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"NEURAL_CHIPS")]
        NEURAL_CHIPS = 60,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOOD_REGULATORS")]
        MOOD_REGULATORS = 61,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"VIRAL_AGENTS")]
        VIRAL_AGENTS = 62,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MICRO_FUSION_GENERATORS")]
        MICRO_FUSION_GENERATORS = 63,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SUPERGRAINS")]
        SUPERGRAINS = 64,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"LASER_RIFLES")]
        LASER_RIFLES = 65,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"HOLOGRAPHICS")]
        HOLOGRAPHICS = 66,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_SALVAGE")]
        SHIP_SALVAGE = 67,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"RELIC_TECH")]
        RELIC_TECH = 68,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"NOVEL_LIFEFORMS")]
        NOVEL_LIFEFORMS = 69,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BOTANICAL_SPECIMENS")]
        BOTANICAL_SPECIMENS = 70,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CULTURAL_ARTIFACTS")]
        CULTURAL_ARTIFACTS = 71,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_PROBE")]
        FRAME_PROBE = 72,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_DRONE")]
        FRAME_DRONE = 73,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_INTERCEPTOR")]
        FRAME_INTERCEPTOR = 74,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_RACER")]
        FRAME_RACER = 75,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_FIGHTER")]
        FRAME_FIGHTER = 76,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_FRIGATE")]
        FRAME_FRIGATE = 77,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_SHUTTLE")]
        FRAME_SHUTTLE = 78,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_EXPLORER")]
        FRAME_EXPLORER = 79,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_MINER")]
        FRAME_MINER = 80,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_LIGHT_FREIGHTER")]
        FRAME_LIGHT_FREIGHTER = 81,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_HEAVY_FREIGHTER")]
        FRAME_HEAVY_FREIGHTER = 82,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_TRANSPORT")]
        FRAME_TRANSPORT = 83,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_DESTROYER")]
        FRAME_DESTROYER = 84,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_CRUISER")]
        FRAME_CRUISER = 85,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_CARRIER")]
        FRAME_CARRIER = 86,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REACTOR_SOLAR_I")]
        REACTOR_SOLAR_I = 87,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REACTOR_FUSION_I")]
        REACTOR_FUSION_I = 88,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REACTOR_FISSION_I")]
        REACTOR_FISSION_I = 89,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REACTOR_CHEMICAL_I")]
        REACTOR_CHEMICAL_I = 90,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REACTOR_ANTIMATTER_I")]
        REACTOR_ANTIMATTER_I = 91,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ENGINE_IMPULSE_DRIVE_I")]
        ENGINE_IMPULSE_DRIVE_I = 92,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ENGINE_ION_DRIVE_I")]
        ENGINE_ION_DRIVE_I = 93,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ENGINE_ION_DRIVE_II")]
        ENGINE_ION_DRIVE_II = 94,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ENGINE_HYPER_DRIVE_I")]
        ENGINE_HYPER_DRIVE_I = 95,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_MINERAL_PROCESSOR_I")]
        MODULE_MINERAL_PROCESSOR_I = 96,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_GAS_PROCESSOR_I")]
        MODULE_GAS_PROCESSOR_I = 97,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_CARGO_HOLD_I")]
        MODULE_CARGO_HOLD_I = 98,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_CARGO_HOLD_II")]
        MODULE_CARGO_HOLD_II = 99,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_CARGO_HOLD_III")]
        MODULE_CARGO_HOLD_III = 100,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_CREW_QUARTERS_I")]
        MODULE_CREW_QUARTERS_I = 101,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_ENVOY_QUARTERS_I")]
        MODULE_ENVOY_QUARTERS_I = 102,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_PASSENGER_CABIN_I")]
        MODULE_PASSENGER_CABIN_I = 103,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_MICRO_REFINERY_I")]
        MODULE_MICRO_REFINERY_I = 104,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_SCIENCE_LAB_I")]
        MODULE_SCIENCE_LAB_I = 105,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_JUMP_DRIVE_I")]
        MODULE_JUMP_DRIVE_I = 106,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_JUMP_DRIVE_II")]
        MODULE_JUMP_DRIVE_II = 107,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_JUMP_DRIVE_III")]
        MODULE_JUMP_DRIVE_III = 108,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_WARP_DRIVE_I")]
        MODULE_WARP_DRIVE_I = 109,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_WARP_DRIVE_II")]
        MODULE_WARP_DRIVE_II = 110,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_WARP_DRIVE_III")]
        MODULE_WARP_DRIVE_III = 111,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_SHIELD_GENERATOR_I")]
        MODULE_SHIELD_GENERATOR_I = 112,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_SHIELD_GENERATOR_II")]
        MODULE_SHIELD_GENERATOR_II = 113,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_ORE_REFINERY_I")]
        MODULE_ORE_REFINERY_I = 114,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_FUEL_REFINERY_I")]
        MODULE_FUEL_REFINERY_I = 115,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_GAS_SIPHON_I")]
        MOUNT_GAS_SIPHON_I = 116,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_GAS_SIPHON_II")]
        MOUNT_GAS_SIPHON_II = 117,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_GAS_SIPHON_III")]
        MOUNT_GAS_SIPHON_III = 118,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SURVEYOR_I")]
        MOUNT_SURVEYOR_I = 119,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SURVEYOR_II")]
        MOUNT_SURVEYOR_II = 120,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SURVEYOR_III")]
        MOUNT_SURVEYOR_III = 121,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SENSOR_ARRAY_I")]
        MOUNT_SENSOR_ARRAY_I = 122,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SENSOR_ARRAY_II")]
        MOUNT_SENSOR_ARRAY_II = 123,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SENSOR_ARRAY_III")]
        MOUNT_SENSOR_ARRAY_III = 124,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_MINING_LASER_I")]
        MOUNT_MINING_LASER_I = 125,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_MINING_LASER_II")]
        MOUNT_MINING_LASER_II = 126,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_MINING_LASER_III")]
        MOUNT_MINING_LASER_III = 127,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_LASER_CANNON_I")]
        MOUNT_LASER_CANNON_I = 128,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_MISSILE_LAUNCHER_I")]
        MOUNT_MISSILE_LAUNCHER_I = 129,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_TURRET_I")]
        MOUNT_TURRET_I = 130,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_PROBE")]
        SHIP_PROBE = 131,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_MINING_DRONE")]
        SHIP_MINING_DRONE = 132,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_SIPHON_DRONE")]
        SHIP_SIPHON_DRONE = 133,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_INTERCEPTOR")]
        SHIP_INTERCEPTOR = 134,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_LIGHT_HAULER")]
        SHIP_LIGHT_HAULER = 135,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_COMMAND_FRIGATE")]
        SHIP_COMMAND_FRIGATE = 136,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_EXPLORER")]
        SHIP_EXPLORER = 137,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_HEAVY_FREIGHTER")]
        SHIP_HEAVY_FREIGHTER = 138,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_LIGHT_SHUTTLE")]
        SHIP_LIGHT_SHUTTLE = 139,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_ORE_HOUND")]
        SHIP_ORE_HOUND = 140,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_REFINING_FREIGHTER")]
        SHIP_REFINING_FREIGHTER = 141,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIP_SURVEYOR")]
        SHIP_SURVEYOR = 142,

    }

    /// <summary>
    /// A waypoint is a location that ships can travel to such as a Planet, Moon or Space Station.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Waypoint
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string Symbol { get; set; }

        [Newtonsoft.Json.JsonProperty("type", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public WaypointType Type { get; set; }

        [Newtonsoft.Json.JsonProperty("systemSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string SystemSymbol { get; set; }

        /// <summary>
        /// Relative position of the waypoint on the system's x axis. This is not an absolute position in the universe.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("x", Required = Newtonsoft.Json.Required.Always)]
        public int X { get; set; }

        /// <summary>
        /// Relative position of the waypoint on the system's y axis. This is not an absolute position in the universe.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("y", Required = Newtonsoft.Json.Required.Always)]
        public int Y { get; set; }

        /// <summary>
        /// Waypoints that orbit this waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("orbitals", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<WaypointOrbital> Orbitals { get; set; } = new gSystem.Collections.ObjectModel.Collection<WaypointOrbital>();

        /// <summary>
        /// The symbol of the parent waypoint, if this waypoint is in orbit around another waypoint. Otherwise this value is undefined.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("orbits", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        public string Orbits { get; set; }

        [Newtonsoft.Json.JsonProperty("faction", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public WaypointFaction Faction { get; set; }

        /// <summary>
        /// The traits of the waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("traits", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<WaypointTrait> Traits { get; set; } = new gSystem.Collections.ObjectModel.Collection<WaypointTrait>();

        /// <summary>
        /// The modifiers of the waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("modifiers", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public gSystem.Collections.Generic.ICollection<WaypointModifier> Modifiers { get; set; }

        [Newtonsoft.Json.JsonProperty("chart", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public Chart Chart { get; set; }

        /// <summary>
        /// True if the waypoint is under construction.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("isUnderConstruction", Required = Newtonsoft.Json.Required.Always)]
        public bool IsUnderConstruction { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The faction that controls the waypoint.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class WaypointFaction
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public FactionSymbol Symbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class WaypointModifier
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public WaypointModifierSymbol Symbol { get; set; }

        /// <summary>
        /// The name of the trait.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        /// <summary>
        /// A description of the trait.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Description { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The unique identifier of the modifier.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum WaypointModifierSymbol
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"STRIPPED")]
        STRIPPED = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"UNSTABLE")]
        UNSTABLE = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"RADIATION_LEAK")]
        RADIATION_LEAK = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CRITICAL_LIMIT")]
        CRITICAL_LIMIT = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CIVIL_UNREST")]
        CIVIL_UNREST = 4,

    }

    /// <summary>
    /// An orbital is another waypoint that orbits a parent waypoint.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class WaypointOrbital
    {
        /// <summary>
        /// The symbol of the orbiting waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public string Symbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class WaypointTrait
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public WaypointTraitSymbol Symbol { get; set; }

        /// <summary>
        /// The name of the trait.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        /// <summary>
        /// A description of the trait.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Description { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// The unique identifier of the trait.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum WaypointTraitSymbol
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"UNCHARTED")]
        UNCHARTED = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"UNDER_CONSTRUCTION")]
        UNDER_CONSTRUCTION = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MARKETPLACE")]
        MARKETPLACE = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHIPYARD")]
        SHIPYARD = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"OUTPOST")]
        OUTPOST = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SCATTERED_SETTLEMENTS")]
        SCATTERED_SETTLEMENTS = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SPRAWLING_CITIES")]
        SPRAWLING_CITIES = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MEGA_STRUCTURES")]
        MEGA_STRUCTURES = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PIRATE_BASE")]
        PIRATE_BASE = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"OVERCROWDED")]
        OVERCROWDED = 9,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"HIGH_TECH")]
        HIGH_TECH = 10,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CORRUPT")]
        CORRUPT = 11,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BUREAUCRATIC")]
        BUREAUCRATIC = 12,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"TRADING_HUB")]
        TRADING_HUB = 13,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"INDUSTRIAL")]
        INDUSTRIAL = 14,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BLACK_MARKET")]
        BLACK_MARKET = 15,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"RESEARCH_FACILITY")]
        RESEARCH_FACILITY = 16,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MILITARY_BASE")]
        MILITARY_BASE = 17,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SURVEILLANCE_OUTPOST")]
        SURVEILLANCE_OUTPOST = 18,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXPLORATION_OUTPOST")]
        EXPLORATION_OUTPOST = 19,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MINERAL_DEPOSITS")]
        MINERAL_DEPOSITS = 20,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"COMMON_METAL_DEPOSITS")]
        COMMON_METAL_DEPOSITS = 21,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PRECIOUS_METAL_DEPOSITS")]
        PRECIOUS_METAL_DEPOSITS = 22,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"RARE_METAL_DEPOSITS")]
        RARE_METAL_DEPOSITS = 23,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"METHANE_POOLS")]
        METHANE_POOLS = 24,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ICE_CRYSTALS")]
        ICE_CRYSTALS = 25,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXPLOSIVE_GASES")]
        EXPLOSIVE_GASES = 26,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"STRONG_MAGNETOSPHERE")]
        STRONG_MAGNETOSPHERE = 27,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"VIBRANT_AURORAS")]
        VIBRANT_AURORAS = 28,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SALT_FLATS")]
        SALT_FLATS = 29,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CANYONS")]
        CANYONS = 30,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PERPETUAL_DAYLIGHT")]
        PERPETUAL_DAYLIGHT = 31,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PERPETUAL_OVERCAST")]
        PERPETUAL_OVERCAST = 32,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DRY_SEABEDS")]
        DRY_SEABEDS = 33,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MAGMA_SEAS")]
        MAGMA_SEAS = 34,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SUPERVOLCANOES")]
        SUPERVOLCANOES = 35,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ASH_CLOUDS")]
        ASH_CLOUDS = 36,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"VAST_RUINS")]
        VAST_RUINS = 37,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MUTATED_FLORA")]
        MUTATED_FLORA = 38,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"TERRAFORMED")]
        TERRAFORMED = 39,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXTREME_TEMPERATURES")]
        EXTREME_TEMPERATURES = 40,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXTREME_PRESSURE")]
        EXTREME_PRESSURE = 41,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DIVERSE_LIFE")]
        DIVERSE_LIFE = 42,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SCARCE_LIFE")]
        SCARCE_LIFE = 43,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FOSSILS")]
        FOSSILS = 44,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"WEAK_GRAVITY")]
        WEAK_GRAVITY = 45,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"STRONG_GRAVITY")]
        STRONG_GRAVITY = 46,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CRUSHING_GRAVITY")]
        CRUSHING_GRAVITY = 47,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"TOXIC_ATMOSPHERE")]
        TOXIC_ATMOSPHERE = 48,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"CORROSIVE_ATMOSPHERE")]
        CORROSIVE_ATMOSPHERE = 49,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BREATHABLE_ATMOSPHERE")]
        BREATHABLE_ATMOSPHERE = 50,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"THIN_ATMOSPHERE")]
        THIN_ATMOSPHERE = 51,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"JOVIAN")]
        JOVIAN = 52,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ROCKY")]
        ROCKY = 53,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"VOLCANIC")]
        VOLCANIC = 54,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FROZEN")]
        FROZEN = 55,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SWAMP")]
        SWAMP = 56,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"BARREN")]
        BARREN = 57,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"TEMPERATE")]
        TEMPERATE = 58,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"JUNGLE")]
        JUNGLE = 59,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"OCEAN")]
        OCEAN = 60,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"RADIOACTIVE")]
        RADIOACTIVE = 61,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MICRO_GRAVITY_ANOMALIES")]
        MICRO_GRAVITY_ANOMALIES = 62,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DEBRIS_CLUSTER")]
        DEBRIS_CLUSTER = 63,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DEEP_CRATERS")]
        DEEP_CRATERS = 64,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHALLOW_CRATERS")]
        SHALLOW_CRATERS = 65,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"UNSTABLE_COMPOSITION")]
        UNSTABLE_COMPOSITION = 66,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"HOLLOWED_INTERIOR")]
        HOLLOWED_INTERIOR = 67,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"STRIPPED")]
        STRIPPED = 68,

    }

    /// <summary>
    /// The type of waypoint.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum WaypointType
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PLANET")]
        PLANET = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"GAS_GIANT")]
        GAS_GIANT = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOON")]
        MOON = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ORBITAL_STATION")]
        ORBITAL_STATION = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"JUMP_GATE")]
        JUMP_GATE = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ASTEROID_FIELD")]
        ASTEROID_FIELD = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ASTEROID")]
        ASTEROID = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ENGINEERED_ASTEROID")]
        ENGINEERED_ASTEROID = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ASTEROID_BASE")]
        ASTEROID_BASE = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"NEBULA")]
        NEBULA = 9,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DEBRIS_FIELD")]
        DEBRIS_FIELD = 10,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"GRAVITY_WELL")]
        GRAVITY_WELL = 11,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ARTIFICIAL_GRAVITY_WELL")]
        ARTIFICIAL_GRAVITY_WELL = 12,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FUEL_STATION")]
        FUEL_STATION = 13,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class RegisterBody
    {
        [Newtonsoft.Json.JsonProperty("faction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [gSystem.ComponentModel.DataAnnotations.StringLength(int.MaxValue, MinimumLength = 1)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public FactionSymbol Faction { get; set; }

        /// <summary>
        /// Your desired agent symbol. This will be a unique name used to represent your agent, and will be the prefix for your ships.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        [gSystem.ComponentModel.DataAnnotations.StringLength(14, MinimumLength = 3)]
        public string Symbol { get; set; }

        /// <summary>
        /// Your email address. This is used if you reserved your call sign between resets.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("email", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public string Email { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body2
    {
        /// <summary>
        /// Symbol of the ship to use.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string ShipSymbol { get; set; }

        /// <summary>
        /// The symbol of the good to supply.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("tradeSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string TradeSymbol { get; set; }

        /// <summary>
        /// Amount of units to supply.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        public int Units { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body3
    {
        /// <summary>
        /// Symbol of a ship located in the destination to deliver a contract and that has a good to deliver in its cargo.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string ShipSymbol { get; set; }

        /// <summary>
        /// The symbol of the good to deliver.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("tradeSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string TradeSymbol { get; set; }

        /// <summary>
        /// Amount of units to deliver.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        public int Units { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body4
    {
        [Newtonsoft.Json.JsonProperty("shipType", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipType ShipType { get; set; }

        /// <summary>
        /// The symbol of the waypoint you want to purchase the ship at.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("waypointSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string WaypointSymbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body5
    {
        /// <summary>
        /// The type of good to produce out of the refining process.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("produce", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public Body5Produce Produce { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body6
    {
        [Newtonsoft.Json.JsonProperty("survey", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.Obsolete]
        public Survey Survey { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body7
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public TradeSymbol Symbol { get; set; }

        /// <summary>
        /// Amount of units to jettison of this good.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(1, int.MaxValue)]
        public int Units { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body8
    {
        /// <summary>
        /// The symbol of the waypoint to jump to. The destination must be a connected waypoint.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("waypointSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string WaypointSymbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body9
    {
        /// <summary>
        /// The target destination.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("waypointSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string WaypointSymbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body10
    {
        [Newtonsoft.Json.JsonProperty("flightMode", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipNavFlightMode FlightMode { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body11
    {
        /// <summary>
        /// The target destination.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("waypointSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string WaypointSymbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class SellCargoRequest
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public TradeSymbol Symbol { get; set; }

        /// <summary>
        /// Amounts of units to sell of the selected good.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        public int Units { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body12
    {
        /// <summary>
        /// The amount of fuel to fill in the ship's tanks. When not specified, the ship will be refueled to its maximum fuel capacity. If the amount specified is greater than the ship's remaining capacity, the ship will only be refueled to its maximum fuel capacity. The amount specified is not in market units but in ship fuel units.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        [gSystem.ComponentModel.DataAnnotations.Range(1, int.MaxValue)]
        public int Units { get; set; }

        /// <summary>
        /// Wether to use the FUEL thats in your cargo or not. Default: false
        /// </summary>
        [Newtonsoft.Json.JsonProperty("fromCargo", Required = Newtonsoft.Json.Required.DisallowNull, NullValueHandling = Newtonsoft.Json.NullValueHandling.Ignore)]
        public bool FromCargo { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body13
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public TradeSymbol Symbol { get; set; }

        /// <summary>
        /// Amounts of units to purchase.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        public int Units { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body14
    {
        [Newtonsoft.Json.JsonProperty("tradeSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public TradeSymbol TradeSymbol { get; set; }

        /// <summary>
        /// Amount of units to transfer.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        public int Units { get; set; }

        /// <summary>
        /// The symbol of the ship to transfer to.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("shipSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string ShipSymbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body15
    {
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Symbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Body16
    {
        /// <summary>
        /// The symbol of the mount to remove.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Symbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response
    {
        /// <summary>
        /// The current status of the game server.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("status", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Status { get; set; }

        /// <summary>
        /// The current version of the API.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("version", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Version { get; set; }

        /// <summary>
        /// The date when the game server was last reset.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("resetDate", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string ResetDate { get; set; }

        [Newtonsoft.Json.JsonProperty("description", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Description { get; set; }

        [Newtonsoft.Json.JsonProperty("stats", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Stats Stats { get; set; } = new Stats();

        [Newtonsoft.Json.JsonProperty("leaderboards", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Leaderboards Leaderboards { get; set; } = new Leaderboards();

        [Newtonsoft.Json.JsonProperty("serverResets", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ServerResets ServerResets { get; set; } = new ServerResets();

        [Newtonsoft.Json.JsonProperty("announcements", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<Announcements> Announcements { get; set; } = new gSystem.Collections.ObjectModel.Collection<Announcements>();

        [Newtonsoft.Json.JsonProperty("links", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<Links> Links { get; set; } = new gSystem.Collections.ObjectModel.Collection<Links>();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response2
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data Data { get; set; } = new Data();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response3
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<System> Data { get; set; } = new gSystem.Collections.ObjectModel.Collection<System>();

        [Newtonsoft.Json.JsonProperty("meta", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Meta Meta { get; set; } = new Meta();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response4
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public System Data { get; set; } = new System();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response5
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<Waypoint> Data { get; set; } = new gSystem.Collections.ObjectModel.Collection<Waypoint>();

        [Newtonsoft.Json.JsonProperty("meta", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Meta Meta { get; set; } = new Meta();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response6
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Waypoint Data { get; set; } = new Waypoint();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response7
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Market Data { get; set; } = new Market();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response8
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Shipyard Data { get; set; } = new Shipyard();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response9
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public JumpGate Data { get; set; } = new JumpGate();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response10
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Construction Data { get; set; } = new Construction();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response11
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data2 Data { get; set; } = new Data2();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response12
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<Faction> Data { get; set; } = new gSystem.Collections.ObjectModel.Collection<Faction>();

        [Newtonsoft.Json.JsonProperty("meta", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Meta Meta { get; set; } = new Meta();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response13
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Faction Data { get; set; } = new Faction();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response14
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Data { get; set; } = new Agent();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response15
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<Agent> Data { get; set; } = new gSystem.Collections.ObjectModel.Collection<Agent>();

        [Newtonsoft.Json.JsonProperty("meta", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Meta Meta { get; set; } = new Meta();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response16
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Data { get; set; } = new Agent();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response17
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<Contract> Data { get; set; } = new gSystem.Collections.ObjectModel.Collection<Contract>();

        [Newtonsoft.Json.JsonProperty("meta", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Meta Meta { get; set; } = new Meta();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response18
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Contract Data { get; set; } = new Contract();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response19
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data3 Data { get; set; } = new Data3();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response20
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data4 Data { get; set; } = new Data4();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response21
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data5 Data { get; set; } = new Data5();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response22
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<Ship> Data { get; set; } = new gSystem.Collections.ObjectModel.Collection<Ship>();

        [Newtonsoft.Json.JsonProperty("meta", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Meta Meta { get; set; } = new Meta();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response23
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data6 Data { get; set; } = new Data6();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response24
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Ship Data { get; set; } = new Ship();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response25
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Data { get; set; } = new ShipCargo();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response26
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data7 Data { get; set; } = new Data7();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response27
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data8 Data { get; set; } = new Data8();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response28
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data9 Data { get; set; } = new Data9();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response29
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Cooldown Data { get; set; } = new Cooldown();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response30
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data10 Data { get; set; } = new Data10();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response31
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data11 Data { get; set; } = new Data11();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response32
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data12 Data { get; set; } = new Data12();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response33
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data13 Data { get; set; } = new Data13();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response34
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data14 Data { get; set; } = new Data14();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response35
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data15 Data { get; set; } = new Data15();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response36
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data16 Data { get; set; } = new Data16();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response37
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data17 Data { get; set; } = new Data17();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response38
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNav Data { get; set; } = new ShipNav();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response39
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNav Data { get; set; } = new ShipNav();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response40
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data18 Data { get; set; } = new Data18();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response41
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data19 Data { get; set; } = new Data19();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response42
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data20 Data { get; set; } = new Data20();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response43
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data21 Data { get; set; } = new Data21();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response44
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data22 Data { get; set; } = new Data22();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response45
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data23 Data { get; set; } = new Data23();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response46
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data24 Data { get; set; } = new Data24();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response47
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data25 Data { get; set; } = new Data25();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response48
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data26 Data { get; set; } = new Data26();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response49
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ShipMount> Data { get; set; } = new gSystem.Collections.ObjectModel.Collection<ShipMount>();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response50
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data27 Data { get; set; } = new Data27();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Response51
    {
        [Newtonsoft.Json.JsonProperty("data", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Data28 Data { get; set; } = new Data28();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ContractType
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PROCUREMENT")]
        PROCUREMENT = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"TRANSPORT")]
        TRANSPORT = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SHUTTLE")]
        SHUTTLE = 2,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum MarketTradeGoodType
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXPORT")]
        EXPORT = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"IMPORT")]
        IMPORT = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"EXCHANGE")]
        EXCHANGE = 2,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum MarketTransactionType
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PURCHASE")]
        PURCHASE = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SELL")]
        SELL = 1,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Frame
    {
        /// <summary>
        /// The symbol of the frame.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Symbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Reactor
    {
        /// <summary>
        /// The symbol of the reactor.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Symbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Engine
    {
        /// <summary>
        /// The symbol of the engine.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Symbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    /// <summary>
    /// A mount on the ship.
    /// </summary>
    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Mounts
    {
        /// <summary>
        /// The symbol of the mount.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("symbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Symbol { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ShipCrewRotation
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"STRICT")]
        STRICT = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"RELAXED")]
        RELAXED = 1,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ShipEngineSymbol
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ENGINE_IMPULSE_DRIVE_I")]
        ENGINE_IMPULSE_DRIVE_I = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ENGINE_ION_DRIVE_I")]
        ENGINE_ION_DRIVE_I = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ENGINE_ION_DRIVE_II")]
        ENGINE_ION_DRIVE_II = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ENGINE_HYPER_DRIVE_I")]
        ENGINE_HYPER_DRIVE_I = 3,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ShipFrameSymbol
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_PROBE")]
        FRAME_PROBE = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_DRONE")]
        FRAME_DRONE = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_INTERCEPTOR")]
        FRAME_INTERCEPTOR = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_RACER")]
        FRAME_RACER = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_FIGHTER")]
        FRAME_FIGHTER = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_FRIGATE")]
        FRAME_FRIGATE = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_SHUTTLE")]
        FRAME_SHUTTLE = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_EXPLORER")]
        FRAME_EXPLORER = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_MINER")]
        FRAME_MINER = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_LIGHT_FREIGHTER")]
        FRAME_LIGHT_FREIGHTER = 9,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_HEAVY_FREIGHTER")]
        FRAME_HEAVY_FREIGHTER = 10,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_TRANSPORT")]
        FRAME_TRANSPORT = 11,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_DESTROYER")]
        FRAME_DESTROYER = 12,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_CRUISER")]
        FRAME_CRUISER = 13,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FRAME_CARRIER")]
        FRAME_CARRIER = 14,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Consumed
    {
        /// <summary>
        /// The amount of fuel consumed by the most recent transit or action.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("amount", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Range(0, int.MaxValue)]
        public int Amount { get; set; }

        /// <summary>
        /// The time at which the fuel was consumed.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("timestamp", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public gSystem.DateTimeOffset Timestamp { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ShipModuleSymbol
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_MINERAL_PROCESSOR_I")]
        MODULE_MINERAL_PROCESSOR_I = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_GAS_PROCESSOR_I")]
        MODULE_GAS_PROCESSOR_I = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_CARGO_HOLD_I")]
        MODULE_CARGO_HOLD_I = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_CARGO_HOLD_II")]
        MODULE_CARGO_HOLD_II = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_CARGO_HOLD_III")]
        MODULE_CARGO_HOLD_III = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_CREW_QUARTERS_I")]
        MODULE_CREW_QUARTERS_I = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_ENVOY_QUARTERS_I")]
        MODULE_ENVOY_QUARTERS_I = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_PASSENGER_CABIN_I")]
        MODULE_PASSENGER_CABIN_I = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_MICRO_REFINERY_I")]
        MODULE_MICRO_REFINERY_I = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_ORE_REFINERY_I")]
        MODULE_ORE_REFINERY_I = 9,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_FUEL_REFINERY_I")]
        MODULE_FUEL_REFINERY_I = 10,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_SCIENCE_LAB_I")]
        MODULE_SCIENCE_LAB_I = 11,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_JUMP_DRIVE_I")]
        MODULE_JUMP_DRIVE_I = 12,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_JUMP_DRIVE_II")]
        MODULE_JUMP_DRIVE_II = 13,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_JUMP_DRIVE_III")]
        MODULE_JUMP_DRIVE_III = 14,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_WARP_DRIVE_I")]
        MODULE_WARP_DRIVE_I = 15,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_WARP_DRIVE_II")]
        MODULE_WARP_DRIVE_II = 16,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_WARP_DRIVE_III")]
        MODULE_WARP_DRIVE_III = 17,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_SHIELD_GENERATOR_I")]
        MODULE_SHIELD_GENERATOR_I = 18,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODULE_SHIELD_GENERATOR_II")]
        MODULE_SHIELD_GENERATOR_II = 19,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ShipMountSymbol
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_GAS_SIPHON_I")]
        MOUNT_GAS_SIPHON_I = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_GAS_SIPHON_II")]
        MOUNT_GAS_SIPHON_II = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_GAS_SIPHON_III")]
        MOUNT_GAS_SIPHON_III = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SURVEYOR_I")]
        MOUNT_SURVEYOR_I = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SURVEYOR_II")]
        MOUNT_SURVEYOR_II = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SURVEYOR_III")]
        MOUNT_SURVEYOR_III = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SENSOR_ARRAY_I")]
        MOUNT_SENSOR_ARRAY_I = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SENSOR_ARRAY_II")]
        MOUNT_SENSOR_ARRAY_II = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_SENSOR_ARRAY_III")]
        MOUNT_SENSOR_ARRAY_III = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_MINING_LASER_I")]
        MOUNT_MINING_LASER_I = 9,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_MINING_LASER_II")]
        MOUNT_MINING_LASER_II = 10,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_MINING_LASER_III")]
        MOUNT_MINING_LASER_III = 11,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_LASER_CANNON_I")]
        MOUNT_LASER_CANNON_I = 12,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_MISSILE_LAUNCHER_I")]
        MOUNT_MISSILE_LAUNCHER_I = 13,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MOUNT_TURRET_I")]
        MOUNT_TURRET_I = 14,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum Deposits
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"QUARTZ_SAND")]
        QUARTZ_SAND = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SILICON_CRYSTALS")]
        SILICON_CRYSTALS = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PRECIOUS_STONES")]
        PRECIOUS_STONES = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ICE_WATER")]
        ICE_WATER = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"AMMONIA_ICE")]
        AMMONIA_ICE = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"IRON_ORE")]
        IRON_ORE = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"COPPER_ORE")]
        COPPER_ORE = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SILVER_ORE")]
        SILVER_ORE = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ALUMINUM_ORE")]
        ALUMINUM_ORE = 8,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"GOLD_ORE")]
        GOLD_ORE = 9,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PLATINUM_ORE")]
        PLATINUM_ORE = 10,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"DIAMONDS")]
        DIAMONDS = 11,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"URANITE_ORE")]
        URANITE_ORE = 12,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MERITIUM_ORE")]
        MERITIUM_ORE = 13,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum ShipReactorSymbol
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REACTOR_SOLAR_I")]
        REACTOR_SOLAR_I = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REACTOR_FUSION_I")]
        REACTOR_FUSION_I = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REACTOR_FISSION_I")]
        REACTOR_FISSION_I = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REACTOR_CHEMICAL_I")]
        REACTOR_CHEMICAL_I = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"REACTOR_ANTIMATTER_I")]
        REACTOR_ANTIMATTER_I = 4,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ShipTypes
    {
        [Newtonsoft.Json.JsonProperty("type", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        [Newtonsoft.Json.JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public ShipType Type { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Crew
    {
        [Newtonsoft.Json.JsonProperty("required", Required = Newtonsoft.Json.Required.Always)]
        public int Required { get; set; }

        [Newtonsoft.Json.JsonProperty("capacity", Required = Newtonsoft.Json.Required.Always)]
        public int Capacity { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum SurveySize
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SMALL")]
        SMALL = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MODERATE")]
        MODERATE = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"LARGE")]
        LARGE = 2,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public enum Body5Produce
    {

        [gSystem.Runtime.Serialization.EnumMember(Value = @"IRON")]
        IRON = 0,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"COPPER")]
        COPPER = 1,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"SILVER")]
        SILVER = 2,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"GOLD")]
        GOLD = 3,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"ALUMINUM")]
        ALUMINUM = 4,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"PLATINUM")]
        PLATINUM = 5,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"URANITE")]
        URANITE = 6,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"MERITIUM")]
        MERITIUM = 7,

        [gSystem.Runtime.Serialization.EnumMember(Value = @"FUEL")]
        FUEL = 8,

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Stats
    {
        /// <summary>
        /// Number of registered agents in the game.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("agents", Required = Newtonsoft.Json.Required.Always)]
        public int Agents { get; set; }

        /// <summary>
        /// Total number of ships in the game.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("ships", Required = Newtonsoft.Json.Required.Always)]
        public int Ships { get; set; }

        /// <summary>
        /// Total number of systems in the game.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("systems", Required = Newtonsoft.Json.Required.Always)]
        public int Systems { get; set; }

        /// <summary>
        /// Total number of waypoints in the game.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("waypoints", Required = Newtonsoft.Json.Required.Always)]
        public int Waypoints { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Leaderboards
    {
        /// <summary>
        /// Top agents with the most credits.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("mostCredits", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<MostCredits> MostCredits { get; set; } = new gSystem.Collections.ObjectModel.Collection<MostCredits>();

        /// <summary>
        /// Top agents with the most charted submitted.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("mostSubmittedCharts", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<MostSubmittedCharts> MostSubmittedCharts { get; set; } = new gSystem.Collections.ObjectModel.Collection<MostSubmittedCharts>();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ServerResets
    {
        /// <summary>
        /// The date and time when the game server will reset.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("next", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Next { get; set; }

        /// <summary>
        /// How often we intend to reset the game server.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("frequency", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Frequency { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Announcements
    {
        [Newtonsoft.Json.JsonProperty("title", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Title { get; set; }

        [Newtonsoft.Json.JsonProperty("body", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Body { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Links
    {
        [Newtonsoft.Json.JsonProperty("name", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Name { get; set; }

        [Newtonsoft.Json.JsonProperty("url", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Url { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data
    {
        [Newtonsoft.Json.JsonProperty("agent", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Agent { get; set; } = new Agent();

        [Newtonsoft.Json.JsonProperty("contract", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Contract Contract { get; set; } = new Contract();

        [Newtonsoft.Json.JsonProperty("faction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Faction Faction { get; set; } = new Faction();

        [Newtonsoft.Json.JsonProperty("ship", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Ship Ship { get; set; } = new Ship();

        /// <summary>
        /// A Bearer token for accessing secured API endpoints.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("token", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string Token { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data2
    {
        [Newtonsoft.Json.JsonProperty("construction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Construction Construction { get; set; } = new Construction();

        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data3
    {
        [Newtonsoft.Json.JsonProperty("agent", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Agent { get; set; } = new Agent();

        [Newtonsoft.Json.JsonProperty("contract", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Contract Contract { get; set; } = new Contract();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data4
    {
        [Newtonsoft.Json.JsonProperty("contract", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Contract Contract { get; set; } = new Contract();

        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data5
    {
        [Newtonsoft.Json.JsonProperty("agent", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Agent { get; set; } = new Agent();

        [Newtonsoft.Json.JsonProperty("contract", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Contract Contract { get; set; } = new Contract();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data6
    {
        [Newtonsoft.Json.JsonProperty("agent", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Agent { get; set; } = new Agent();

        [Newtonsoft.Json.JsonProperty("ship", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Ship Ship { get; set; } = new Ship();

        [Newtonsoft.Json.JsonProperty("transaction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipyardTransaction Transaction { get; set; } = new ShipyardTransaction();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data7
    {
        [Newtonsoft.Json.JsonProperty("nav", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNav Nav { get; set; } = new ShipNav();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data8
    {
        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        [Newtonsoft.Json.JsonProperty("cooldown", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Cooldown Cooldown { get; set; } = new Cooldown();

        /// <summary>
        /// Goods that were produced by this refining process.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("produced", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<Produced> Produced { get; set; } = new gSystem.Collections.ObjectModel.Collection<Produced>();

        /// <summary>
        /// Goods that were consumed during this refining process.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("consumed", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<Consumed2> Consumed { get; set; } = new gSystem.Collections.ObjectModel.Collection<Consumed2>();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data9
    {
        [Newtonsoft.Json.JsonProperty("chart", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Chart Chart { get; set; } = new Chart();

        [Newtonsoft.Json.JsonProperty("waypoint", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Waypoint Waypoint { get; set; } = new Waypoint();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data10
    {
        [Newtonsoft.Json.JsonProperty("nav", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNav Nav { get; set; } = new ShipNav();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data11
    {
        [Newtonsoft.Json.JsonProperty("cooldown", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Cooldown Cooldown { get; set; } = new Cooldown();

        /// <summary>
        /// Surveys created by this action.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("surveys", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<Survey> Surveys { get; set; } = new gSystem.Collections.ObjectModel.Collection<Survey>();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data12
    {
        [Newtonsoft.Json.JsonProperty("cooldown", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Cooldown Cooldown { get; set; } = new Cooldown();

        [Newtonsoft.Json.JsonProperty("extraction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Extraction Extraction { get; set; } = new Extraction();

        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data13
    {
        [Newtonsoft.Json.JsonProperty("cooldown", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Cooldown Cooldown { get; set; } = new Cooldown();

        [Newtonsoft.Json.JsonProperty("siphon", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Siphon Siphon { get; set; } = new Siphon();

        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data14
    {
        [Newtonsoft.Json.JsonProperty("cooldown", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Cooldown Cooldown { get; set; } = new Cooldown();

        [Newtonsoft.Json.JsonProperty("extraction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Extraction Extraction { get; set; } = new Extraction();

        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data15
    {
        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data16
    {
        [Newtonsoft.Json.JsonProperty("nav", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNav Nav { get; set; } = new ShipNav();

        [Newtonsoft.Json.JsonProperty("cooldown", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Cooldown Cooldown { get; set; } = new Cooldown();

        [Newtonsoft.Json.JsonProperty("transaction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public MarketTransaction Transaction { get; set; } = new MarketTransaction();

        [Newtonsoft.Json.JsonProperty("agent", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Agent { get; set; } = new Agent();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data17
    {
        [Newtonsoft.Json.JsonProperty("fuel", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipFuel Fuel { get; set; } = new ShipFuel();

        [Newtonsoft.Json.JsonProperty("nav", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNav Nav { get; set; } = new ShipNav();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data18
    {
        [Newtonsoft.Json.JsonProperty("fuel", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipFuel Fuel { get; set; } = new ShipFuel();

        [Newtonsoft.Json.JsonProperty("nav", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipNav Nav { get; set; } = new ShipNav();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data19
    {
        [Newtonsoft.Json.JsonProperty("agent", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Agent { get; set; } = new Agent();

        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        [Newtonsoft.Json.JsonProperty("transaction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public MarketTransaction Transaction { get; set; } = new MarketTransaction();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data20
    {
        [Newtonsoft.Json.JsonProperty("cooldown", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Cooldown Cooldown { get; set; } = new Cooldown();

        /// <summary>
        /// List of scanned systems.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("systems", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ScannedSystem> Systems { get; set; } = new gSystem.Collections.ObjectModel.Collection<ScannedSystem>();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data21
    {
        [Newtonsoft.Json.JsonProperty("cooldown", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Cooldown Cooldown { get; set; } = new Cooldown();

        /// <summary>
        /// List of scanned waypoints.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("waypoints", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ScannedWaypoint> Waypoints { get; set; } = new gSystem.Collections.ObjectModel.Collection<ScannedWaypoint>();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data22
    {
        [Newtonsoft.Json.JsonProperty("cooldown", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Cooldown Cooldown { get; set; } = new Cooldown();

        /// <summary>
        /// List of scanned ships.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("ships", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ScannedShip> Ships { get; set; } = new gSystem.Collections.ObjectModel.Collection<ScannedShip>();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data23
    {
        [Newtonsoft.Json.JsonProperty("agent", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Agent { get; set; } = new Agent();

        [Newtonsoft.Json.JsonProperty("fuel", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipFuel Fuel { get; set; } = new ShipFuel();

        [Newtonsoft.Json.JsonProperty("transaction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public MarketTransaction Transaction { get; set; } = new MarketTransaction();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data24
    {
        [Newtonsoft.Json.JsonProperty("agent", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Agent { get; set; } = new Agent();

        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        [Newtonsoft.Json.JsonProperty("transaction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public MarketTransaction Transaction { get; set; } = new MarketTransaction();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data25
    {
        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data26
    {
        [Newtonsoft.Json.JsonProperty("contract", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Contract Contract { get; set; } = new Contract();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data27
    {
        [Newtonsoft.Json.JsonProperty("agent", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Agent { get; set; } = new Agent();

        /// <summary>
        /// List of installed mounts after the installation of the new mount.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("mounts", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ShipMount> Mounts { get; set; } = new gSystem.Collections.ObjectModel.Collection<ShipMount>();

        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        [Newtonsoft.Json.JsonProperty("transaction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipModificationTransaction Transaction { get; set; } = new ShipModificationTransaction();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Data28
    {
        [Newtonsoft.Json.JsonProperty("agent", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public Agent Agent { get; set; } = new Agent();

        /// <summary>
        /// List of installed mounts after the removal of the selected mount.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("mounts", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public gSystem.Collections.Generic.ICollection<ShipMount> Mounts { get; set; } = new gSystem.Collections.ObjectModel.Collection<ShipMount>();

        [Newtonsoft.Json.JsonProperty("cargo", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipCargo Cargo { get; set; } = new ShipCargo();

        [Newtonsoft.Json.JsonProperty("transaction", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required]
        public ShipModificationTransaction Transaction { get; set; } = new ShipModificationTransaction();

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class MostCredits
    {
        /// <summary>
        /// Symbol of the agent.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("agentSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string AgentSymbol { get; set; }

        /// <summary>
        /// Amount of credits.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("credits", Required = Newtonsoft.Json.Required.Always)]
        public long Credits { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class MostSubmittedCharts
    {
        /// <summary>
        /// Symbol of the agent.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("agentSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string AgentSymbol { get; set; }

        /// <summary>
        /// Amount of charts done by the agent.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("chartCount", Required = Newtonsoft.Json.Required.Always)]
        public int ChartCount { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Produced
    {
        /// <summary>
        /// Symbol of the good.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("tradeSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string TradeSymbol { get; set; }

        /// <summary>
        /// Amount of units of the good.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        public int Units { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NJsonSchema", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class Consumed2
    {
        /// <summary>
        /// Symbol of the good.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("tradeSymbol", Required = Newtonsoft.Json.Required.Always)]
        [gSystem.ComponentModel.DataAnnotations.Required(AllowEmptyStrings = true)]
        public string TradeSymbol { get; set; }

        /// <summary>
        /// Amount of units of the good.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("units", Required = Newtonsoft.Json.Required.Always)]
        public int Units { get; set; }

        private gSystem.Collections.Generic.IDictionary<string, object> _additionalProperties;

        [Newtonsoft.Json.JsonExtensionData]
        public gSystem.Collections.Generic.IDictionary<string, object> AdditionalProperties
        {
            get { return _additionalProperties ?? (_additionalProperties = new gSystem.Collections.Generic.Dictionary<string, object>()); }
            set { _additionalProperties = value; }
        }

    }



    [gSystem.CodeDom.Compiler.GeneratedCode("NSwag", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ApiException : gSystem.Exception
    {
        public int StatusCode { get; private set; }

        public string Response { get; private set; }

        public gSystem.Collections.Generic.IReadOnlyDictionary<string, gSystem.Collections.Generic.IEnumerable<string>> Headers { get; private set; }

        public ApiException(string message, int statusCode, string response, gSystem.Collections.Generic.IReadOnlyDictionary<string, gSystem.Collections.Generic.IEnumerable<string>> headers, gSystem.Exception innerException)
            : base(message + "\n\nStatus: " + statusCode + "\nResponse: \n" + ((response == null) ? "(null)" : response.Substring(0, response.Length >= 512 ? 512 : response.Length)), innerException)
        {
            StatusCode = statusCode;
            Response = response;
            Headers = headers;
        }

        public override string ToString()
        {
            return string.Format("HTTP Response: \n\n{0}\n\n{1}", Response, base.ToString());
        }
    }

    [gSystem.CodeDom.Compiler.GeneratedCode("NSwag", "13.18.2.0 (NJsonSchema v10.8.0.0 (Newtonsoft.Json v11.0.0.0))")]
    public partial class ApiException<TResult> : ApiException
    {
        public TResult Result { get; private set; }

        public ApiException(string message, int statusCode, string response, gSystem.Collections.Generic.IReadOnlyDictionary<string, gSystem.Collections.Generic.IEnumerable<string>> headers, TResult result, gSystem.Exception innerException)
            : base(message, statusCode, response, headers, innerException)
        {
            Result = result;
        }
    }

}

#pragma warning restore 1591
#pragma warning restore 1573
#pragma warning restore 472
#pragma warning restore 114
#pragma warning restore 108
#pragma warning restore 3016
#pragma warning restore 8603